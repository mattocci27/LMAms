...
```{r PA-part-plt, cache=TRUE, comment=NA}
...
GL_sim <- \(LMAs) {
  A <- exp(log(2) +  
           ...
           #0.4 * log(LMAp) +
           GL_para$`X50.`[1] * log(LMAs)
  )
#  A <- rlnorm(N, log(2) +  
#           GL_para$`X50.`[2] * log(LMAp) +
#           GL_para$`X50.`[1] * log(LMAs), 0.13)
...
#cov(log(GL$LMAp), log(GL$LMAs))
#cor(log(GL$LMAp), log(GL$LMAs))
#cor(log(GL$LMAp), log(GL$LMAs)) * sd(log(GL$LMAs)) * sd(log(GL$LMAp))
#
#
#N <- 100
#tmp <- seq(log(1.01), log(10), length = 10)
#LMAs_var_mat <- NULL
#b_mat <- NULL
#for (i in 1:100) {
#  b_val <- NULL
#  LMAs_var <- NULL
#  for (j in 1:10) {
#    S <- matrix(c(0.8^2, 0.18 * 0.8 * tmp[j],
#     0.18 * 0.8 * tmp[j], tmp[j]^2), ncol = 2)
#    LMA_mat <- mvtnorm::rmvnorm(100, c(log(80), log(80)), S)
#    LMAp <- exp(LMA_mat[,1])
#    LMAs <- exp(LMA_mat[,2])
#    LMA <- LMAp + LMAs
#    LMAs_var <- c(LMAs_var, cov(LMA, LMAs) / var(LMA)) 
#    b_val <- c(b_val, GL_sim(LMAs))
#  }
#  LMAs_var_mat <- cbind(LMAs_var_mat, LMAs_var)
#  b_mat <- cbind(b_mat, b_val)
#}
#
#
#LMAs_var_mean <- apply(LMAs_var_mat, 1, mean)
#mean_ <- apply(b_mat, 1, mean)
#upr_ <- apply(b_mat, 1, \(x)(quantile(x, 0.975)))
#lwr_ <- apply(b_mat, 1, \(x)(quantile(x, 0.025)))
#
#b_dat <- tibble(mean_, upr_, lwr_, LMAs_var_mean = LMAs_var_mean * 100)
#
##plot(c(LMAs_var_mat), c(b_mat))
#
...
LMAs_var <- NULL
for (i in 1:1000) {
  LMAp <- rlnorm(N, log(40), 1)
  tmp <- seq(log(1.01), log(10), length = 10)
  LMAs <- map(tmp, \(x) rlnorm(N, log(40), x))
  LMAs_var <- cbind(LMAs_var, map_dbl(LMAs, var_fun))
  b_GL <- cbind(b_val, map_dbl(LMAs, GL_sim))
  b_PA <- cbind(b_val, map_dbl(LMAs, PA_sim))
}


b_dat <- as.data.frame(b_val)
LMAs_var_mean <- apply(LMAs_var, 1, mean)
mean_ <- apply(b_dat, 1, mean)
upr_ <- apply(b_dat, 1, \(x)(quantile(x, 0.975)))
lwr_ <- apply(b_dat, 1, \(x)(quantile(x, 0.025)))
...
for (i in 1:1000) {
  ...
  LMAs <- map(tmp, \(x) rlnorm(N, log(40), x))
  LMAs_var <- cbind(LMAs_var, map_dbl(LMAs, var_fun))
  b_val <- cbind(b_val, map_dbl(LMAs, PA_sim))
}
...
#mass_plt_simple <- ggplot(b_dat2, aes(x = exp(var_)^2)) +
mass_plt_simple2 <- ggplot(b_dat2) +
#mass_plt_simple <- ggplot(b_dat2, aes(x = var_)) +
  geom_ribbon(aes(ymin = lwr_, ymax = upr_, x = LMAs_var_mean), fill = "grey70")  +
  geom_line(aes(y = mean_, x = LMAs_var_mean)) +
...
N <- 100
b_val <- NULL
tmp1 <- seq(log(1.01), log(2), length = 20)
tmp2 <- seq(log(2), log(10), length = 20)
tmp <- c(tmp1, tmp2)
for (i in 1:1000) {
  LMAp <- rlnorm(N, log(40), 1)
#  tmp <- seq(1.01, 10, length = 20) |> log()
  LMAs <- map(tmp, \(x) rlnorm(N, log(40), x))
  b_val <- cbind(b_val, map_dbl(LMAs, hoge))
}
...
b_dat2 <- tibble(mean_, upr_, lwr_, var_ = tmp, var2 = tmp2)

mass_plt_simple2 <- ggplot(b_dat2, aes(x = var2)) +
  geom_vline(xintercept = var_LMAp, lty = 2) +
  geom_ribbon(aes(ymin = lwr_, ymax = upr_), fill = "grey70")  +
  geom_line(aes(y = mean_)) +
#  geom_point(aes(y = mean_)) +
  ...
  ylab(expression(Mass~proportionality~(italic(b)))) +
  xlab(expression(Variance~of~LMAs))
  #xlab("Shape parameter of LMAs")
mass_plt_simple2
...
![Mass-proportionality-simple](../figs/mass_prop_simple.png)

![Mass-proportionality-simple2](../figs/mass_prop_simple2.png)
...
var_ <- NULL
for (i in 1:1000) {
  y <- rlnorm(100, log_mu, log_sig)
  mean_[i] <- mean(y)
  var_[i] <- var(y)
}

exp(log_mu + log_sig^2 / 2)
(exp(log_sig^2) - 1) * exp(2 * log_mu + log_sig^2)
...
```{r mass-prop-ex, cache=TRUE, comment=NA}

mass_plt_SI <- ggplot(dat_ex %>% filter(am >= 0.1), aes(x = am, y= as, fill = val)) +
  geom_raster() +
  facet_grid(~type) +
  scale_fill_gradient2(midpoint = 0.5,
                         low = "#e66101",
...
p <- plot_grid(mass_plt_SI,
               legend_plt2,
               nrow = 2,
               rel_heights = c(.6, .4))

my_ggsave("../figs/mass_prop_SI.png", p,
          width = 12,
          height = 8)
```

![Mass-proportionality3](../figs/mass_prop_SI.png)
