...
$$
A_{area\ i} = \alpha_0 LMAm_{i}^{\alpha_m} LDs_{i}^{\alpha_s} \\
LL_{i} = \beta_0 LMAm_{i}^{\beta_m} LDs_{i}^{\beta_s} exp(\theta Light_i) \\
R_{area\ i} = \gamma_0 LMAm_{i}^{\gamma_m} LDs_{i}^{\gamma_s}
$$
...
load("./data/PA_latent_replusive_model_light2_LT_obs.rda")
PA <- dat %>%
  as_data_frame %>%
...
p_est <- mcmc_res[,paste0("p.",  1:106)] %>% apply(.,2,mean)
