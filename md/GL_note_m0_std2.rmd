...
temp_LMAp <- rstan::extract(res, "log_LMAp_ori")[[1]][5,] %>% exp
temp_LMAs <- rstan::extract(res, "log_LMAs_ori")[[1]][5,] %>% exp
...
log_LMAp <- rstan::extract(res, "log_LMAp_ori")[[1]]
log_LMAs <- rstan::extract(res, "log_LMAs_ori")[[1]]
...
apply(log_LMAs, 1, function(x)cor(x, log(GL$Aarea))) %>% hist(main = "LMAp vs Aarea, R")


apply(log_LMAp, 1, function(x)cor(x, log(GL$Rarea))) %>% hist(main = "LMAp vs Rarea, R")
apply(log_LMAs, 1, function(x)cor(x, log(GL$Rarea))) %>% hist(main = "LMAp vs Rarea, R")


apply(log_LMAp, 1, function(x)cor.test(x, log(GL$Narea))$estimate) %>% hist(main = "LMAp vs Narea, R")
apply(log_LMAs, 1, function(x)cor.test(x, log(GL$Narea))$estimate) %>% hist(main = "LMAp vs Narea, R")

apply(log_LMAp, 1, function(x)cor.test(x, log(GL$Parea))$estimate) %>% hist(main = "LMAp vs Parea, R")
apply(log_LMAs, 1, function(x)cor.test(x, log(GL$Parea))$estimate) %>% hist(main = "LMAp vs Parea, R")
...
ggplot(GL3, aes(x = val, y = val3, fill = gr, col = gr)) +
  geom_point(shape = 21) +
  facet_wrap(trait2 ~ LMA, scales = "free") +
  xlab("LMA") +
  ylab("Traits") +
  scale_fill_manual(values = fills) +
  scale_colour_manual(values = cols) +
