---
title: Panama results using normal distribution with CV
...
load("./data/PA_LMAm_LMAsLT2_obs_cv.rda")

get_beta <- function(model){
  rstan::extract(model, "beta")[[1]] %>%
    apply(., 2, median)
}

get_sigma <- function(model){
  rstan::extract(model, "sigma")[[1]] %>%
    apply(., 2, median)
}


res2 <- res %>%
  mutate(beta = map(model, get_beta)) %>%
  mutate(sigma = map(model, get_sigma))


modcv_fun <- function(data, beta, sigma){
  moge <- data %>% as.data.frame

  list_dat <- list(N = nrow(moge),
                   obs = cbind(log(moge[ , "Aarea"] + moge[ , "Rarea"]),
                               log(moge[ , "LL"]),
                               log(moge[ , "Rarea"])),
                   LMA = moge[ , "LMA"],
                   LT = moge[ , "LT"],
                   ...
                   DE = 1,
                   q_lim = q_lim,
                   #leaf = as.numeric(as.factor(moge$strata)),
                   leaf = ifelse(moge$strata == "CAN", 1, 0),
                   dry = ifelse(moge$site == "PNM", 1 , 0),
                   beta = beta,
                   sigma = sigma
                   )
...
res3 <- res2 %>%
  mutate(model_cv = pmap(list(data, beta, sigma), modcv_fun))
