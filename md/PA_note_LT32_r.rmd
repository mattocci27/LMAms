---
title: Panama results using normal distribution
...
\begin{align}
  ...
  = E[A_{\mathrm{area} \; i}]
  = \alpha_0\mathrm{LMAp}^{\alpha_p} (\mathrm{LMAs}_i / \mathrm{LT}_i) ^{\alpha_s} \\
  &E[\mathrm{LL}_{i}] = \beta_{0} A_{\mathrm{area} \; i}^{\beta_p} (\mathrm{LMAs}_i/\mathrm{LT}_i)^{\beta_s} \mathrm{exp}(\beta_L \rm{L}) \\
...
load("../data/PA_LMAm_LMAsLT32_std_rand.rda")
#summary_PA_LDL <- data.frame(summary(res)$summary)

res <- rand_res$model[[1]]
#summary_PA_LDL <- data.frame(summary(res)$summary)

summary_PA_LDL <- rand_res$summary[[1]]
...
cor.test(log(PA$LL), log(PA$LMAs/PA$LT))
