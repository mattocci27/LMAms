...
load("../data/latent_model_ordered10_more_obs.rda")
#load("./data/GL_latent_repulsive_model_obs.rda")
...
plot(1:m, p_m[,3], type = "l", col ="orange")
...
tmp <- seq(1, 4, length = 100)
tmp2 <- var1(tmp, tmp)
plot(tmp2 ~ tmp, type = "l", ylim = c(0, 0.05))

hist(rbeta(200, 2.2, 2.2))
hist(rnorm(200, 0.5, 0.17))

tmp3 <- numeric(200)
for (i in 1:200) tmp3[i] <- mean(sample(rbeta(4000, 1, 1), 4000))
hist(tmp3)
