...
n <- 1000
p <- array(NA, dim = c(m, n, K))

library(mvtnorm)

for (i in 1:n) {
  p_i <- params$p[,i]
  obs_i <- list_dat$obs[i,] 
  Z_i <- params$Z[i,1:3,1:3]
  Sigma_i <- params$Sigma[i,1:3,1:3]
  L_sigma_i <- params$L_sigma[i,]
  mu_i <- obs_i %*% Z_i  - 0.5 * L_sigma_i^2

  p[,i, 1] <- p_i * dmvnorm(obs_i, mu_i, Sigma_i) / (p_i * dmvnorm(obs_i, mu_i, Sigma_i) + (1 - p_i) * dmvnorm(obs_i, mu_i, Sigma_i))

  p[,i,2] <- (1-p_i) * dmvnorm(obs_i, mu_i, Sigma_i) / (p_i * dmvnorm(obs_i, mu_i, Sigma_i) + (1 -p_i) * dmvnorm(obs_i, mu_i, Sigma_i))
}

plot(1:4000, p[1:4000,1,1], type = "l", col = "orange")
points(1:4000, p[1:4000,1,2], type = "l",col = "violet")

switch <- stephens(p = p)

mcmc_pars <- array(NA, dim = c(m, K, J))
mcmc_pars[,1,1] <- extract(fit, par = "mu1")[[1]]
mcmc_pars[,2,1] <- extract(fit, par = "mu2")[[1]]
mcmc_pars[,1,2] <- extract(fit, par = "sigma[1]")[[1]]
mcmc_pars[,2,2] <- extract(fit, par = "sigma[2]")[[1]]
mcmc_pars[,1,3] <- extract(fit, par = "theta")[[1]]
mcmc_pars[,2,3] <- 1 - mcmc_pars[,1,3] 

mcmc_pars_re <- permute.mcmc(mcmc_pars,switch$permutations)[[1]]

fig_dat <- data_frame(mu1 = mcmc_pars_re[,1,1],
           mu2 = mcmc_pars_re[,2,1],
           sigma1 = mcmc_pars_re[,1,2],
           sigma2 = mcmc_pars_re[,2,2],
           theta = mcmc_pars_re[,1,3],
           iter = 1:m) 
  
fig_dat2<- fig_dat %>% gather(para, val, 1:5)

ggplot(fig_dat2, aes(x=iter,y=val, col = para)) +
  facet_grid(~para) +
  geom_line()

ggplot(fig_dat, aes(x=mu1, y=mu2)) +
  geom_point()


mpc = fit@sim$iter-fit@sim$warmup
tmp = t(apply(mcmc_pars_re,1,as.vector))
amu = array(NA,dim = c(mpc,fit@sim$chains,dim(tmp)[2]))
for (cc in 1:fit@sim$chains) amu[,cc,] = tmp[(1:mpc)+(cc-1)*mpc,]
mmu = monitor(amu, print = F, warmup = 0)

rownames(mmu) <- c("mu1", "mu2", "sigma1", "sigma2", "theta", "theta2") 
...
breaks=(0:50) / 50
prior_hist <- hist(rbeta(1000, 1, 1), breaks=breaks, plot=FALSE)
post_hist <- hist(params1$`p[1]`, breaks=breaks, plot=FALSE)
...
breaks=(0:50) / 50
prior_hist <- hist(rbeta(1000, 1, 1), breaks=breaks, plot=FALSE)
post_hist <- hist(params1$`p[10]`, breaks=breaks, plot=FALSE)
