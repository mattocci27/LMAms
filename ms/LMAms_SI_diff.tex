...
%DIF DEL LMAms_SI_old.tex   Wed Aug 21 10:19:15 2024
%DIF ADD LMAms_SI.tex       Wed Aug 21 10:18:55 2024
...
randomized data to assess potential overfitting.
\DIFaddbegin 
...
\DIFaddend Models fit to the randomized datasets either did not converge or showed
divergent transitions (\DIFdelbegin \DIFdel{Supplementary Section
3}\DIFdelend \DIFaddbegin \DIFadd{Table AS1, Figure AS1-2}\DIFaddend ), indicating that models
fit to randomized data do not provide reliable inferences
(\citeproc{ref-Betancourt2016}{Betancourt, 2016}). Furthermore, when
models were fit to randomized data, the scaling exponents
(\(\alpha_{m,s}\), \(\beta_{m,s}\), and \(\gamma_{m,s}\)) were not
significantly different from zero. In simple terms, the two-dimensional
models failed when fit to randomized data.
\DIFaddbegin 

\DIFaddend In contrast, when fit to the observed (non-randomized) data, the
...
controversial (\citeproc{ref-Westoby2013}{Westoby et al., 2013}).
Although \emph{A}\textsubscript{max} is often mass-normalized
...
area). Osnas et al. (\citeproc{ref-Osnas2018}{2018}) further showed that
the relationship between \emph{A}\textsubscript{max} and LMA (i.e., the
