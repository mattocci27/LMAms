...
%DIF DEL LMAms_SI_old.tex   Tue Jan 28 08:37:14 2025
%DIF ADD LMAms_SI.tex       Tue Jan 28 08:37:02 2025
...
%DIF 190d190
%DIF < \renewcommand{\thesection}{S\arabic{section}}
%DIF -------
\usepackage{fvextra}
\usepackage{multirow}
...
\section{Understanding \DIFaddbegin \DIFadd{and simulating }\DIFaddend relationships between
photosynthetic capacity and
LMA}\DIFdelbegin %DIFDELCMD < \label{understanding-relationships-between-photosynthetic-capacity-and-lma}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{understanding-and-simulating-relationships-between-photosynthetic-capacity-and-lma}
\DIFaddend 
...
reported \DIFdelbegin \DIFdel{estiamtes }\DIFdelend \DIFaddbegin \DIFadd{estimates }\DIFaddend ranging between 0 and 1
(\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Osnas2018}{Osnas et al., 2018}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Osnas2018}{Osnas et al. 2018}}\hspace{0pt}%DIFAUXCMD
...
\DIFdel{= -0.47 between
LMAm and LMAs for Panama shade leaves (see Supplementary Section 6).
...
quantify the relationship between }\emph{\DIFdel{A}}%DIFAUXCMD
\DIFdel{\textsubscript{area} and LMA.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{Simulated datasets to study relationships between
photosynthetic capacity and
LMA}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{simulated-datasets-to-study-relationships-between-photosynthetic-capacity-and-lma}
%DIFDELCMD < 
...
include all leaves in the Panama dataset, whereas Figs. \DIFdelbegin \DIFdel{6-7 }\DIFdelend \DIFaddbegin \DIFadd{5-6 }\DIFaddend in the main
text only include Panama species for which both sun and shade leaves
were available. Boxplot symbols as in Figs. \DIFdelbegin \DIFdel{6-7}\DIFdelend \DIFaddbegin \DIFadd{5-6}\DIFaddend . Groups sharing the same
...
LMAs = (1 -- \emph{f}) \(\times\) LMA. Boxplot symbols as in Fig. \DIFdelbegin \DIFdel{6.
}\DIFdelend \DIFaddbegin \DIFadd{5.
}\DIFaddend Groups sharing the same letters are not significantly different (P
...
leaves were available). Note that LMAm = \emph{f} \(\times\) LMA, and
LMAs = (1 -- \emph{f}) \(\times\) LMA. Boxplot symbols as in Fig. \DIFdelbegin \DIFdel{7.
...
typically between 0 and 1 (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\emph{b} \textgreater{} 0 (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
Supplementary Section \DIFdelbegin \DIFdel{6}\DIFdelend \DIFaddbegin \DIFadd{7}\DIFaddend : \(\alpha_0\) = 1.77, \(\mu_m\) = 56.4,
...
the best Panama model (Table 1) and Supplementary Section \DIFdelbegin \DIFdel{6}\DIFdelend \DIFaddbegin \DIFadd{7}\DIFaddend :
...
}\emph{\DIFadd{P}}\DIFadd{\textsubscript{area} for GLOPNET
(}\quartosuppfigref{suppfig-gl_point_np2}\DIFadd{). The results shown here
include all leaves for which }\emph{\DIFadd{N}}\DIFadd{\textsubscript{area},
...
\DIFdelbegin %DIFDELCMD < \begin{suppfig}
%DIFDELCMD < 

%DIFDELCMD < \centering{
%DIFDELCMD < 

%DIFDELCMD < \includegraphics{../figs/ps_point.pdf}
%DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < \caption{\label{suppfig-LMAm_LMAs}%%%
...
single axis could not accurately represent the two-dimensional space.
Symbols as in main text Figs. 2-3.}%DIFDELCMD < \MBLOCKRIGHTBRACE
%DIFDELCMD < 
