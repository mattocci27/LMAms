...
%DIF DEL LMAms_SI_old.tex   Wed Oct 23 09:22:45 2024
%DIF ADD LMAms_SI.tex       Wed Oct 23 09:17:27 2024
...
%DIF 184-190c184
%DIF < \usepackage{xr}
%DIF < \externaldocument{LMAms_main}
...
\renewcommand{\theequation}{\thesection.\arabic{equation}} %DIF > 
\renewcommand{\thesection}{S\arabic{section}} %DIF > 
%DIF -------
\usepackage{fvextra}
\usepackage{multirow}
%DIF 195-196c193-196
%DIF < \DefineVerbatimEnvironment{Highlighting}{Verbatim}{breaklines,commandchars=\\\{\}}
%DIF < \fancyhead[RE,RO]{Katabuchi \textit{et al}.  Appendix}
%DIF -------
\DefineVerbatimEnvironment{Highlighting}{Verbatim}{fontsize=\small,breaklines,commandchars=\\\{\}} %DIF > 
\usepackage{titling} %DIF > 
\pretitle{\begin{center}\large\textbf} %DIF > 
\posttitle{\end{center}} %DIF > 
%DIF -------
...
%DIFDELCMD < %%%
\DIFdelend \section{\DIFdelbegin \DIFdel{Supplementary Section 1: }\DIFdelend Prior information}\DIFdelbegin %DIFDELCMD < \label{supplementary-section-1-prior-information}
%DIFDELCMD < %%%
...
where E{[}\(\cdot\){]} indicates \DIFaddbegin \DIFadd{the }\DIFaddend expected value and
...
}\DIFaddend ). The covariance matrix in
Eq.\DIFdelbegin \DIFdel{S1 }\DIFdelend \DIFaddbegin \DIFadd{~\ref{eq-mvn} }\DIFaddend was decomposed as
...
\(\boldsymbol{\Sigma}\) directly, priors were assigned \DIFdelbegin \DIFdel{on
}\DIFdelend \DIFaddbegin \DIFadd{to
}\DIFaddend \({\boldsymbol \sigma}\) and \textbf{L} to avoid a strong dependence
between \({\boldsymbol \sigma}\) and \({\boldsymbol \Omega}\)
(\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Alvarez2014}{Alvarez et al., 2014}}\hspace{0pt}%DIFAUXCMD
;
\mbox{%DIFAUXCMD
\citeproc{ref-Lewandowski2009}{Lewandowski et al., 2009}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Lewandowski2009}{Lewandowski D et al. 2009}}\hspace{0pt}%DIFAUXCMD
;
\mbox{%DIFAUXCMD
\citeproc{ref-Alvarez2014}{Alvarez I et al. 2014}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ). A prior for
\textbf{L} was specified as \DIFdelbegin \DIFdel{a so-called }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend LKJ distribution with \DIFdelbegin \DIFdel{shape
parameter }\DIFdelend \DIFaddbegin \DIFadd{a shape parameter
of }\DIFaddend 2 (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Lewandowski2009}{Lewandowski et al., 2009}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Lewandowski2009}{Lewandowski D et al. 2009}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ), which
...
for coefficients near zero (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Gelman2008}{Gelman et al., 2008}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Gelman2008}{Gelman A et al.
2008}}\hspace{0pt}%DIFAUXCMD
...
\section{\DIFdelbegin \DIFdel{Supplementary Section 2: }\DIFdelend Model constraints}\DIFdelbegin %DIFDELCMD < \label{supplementary-section-2-model-constraints}
%DIFDELCMD < %%%
...
\section{\DIFdelbegin \DIFdel{Supplementary Section 3: }\DIFdelend Model tests with randomized data (including Table AS1 and
Figure AS1\DIFaddbegin \DIFadd{,
2}\DIFaddend )}\DIFdelbegin %DIFDELCMD < \label{supplementary-section-3-model-tests-with-randomized-data-including-table-as1-and-figure-as1}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{model-tests-with-randomized-data-including-table-as1-and-figure-as1-2}
\DIFaddend 

Because our two-dimensional (LMAm-LMAs) modeling approach includes many
parameters (\DIFaddbegin \DIFadd{with }\DIFaddend one latent variable \emph{f\textsubscript{i}} to
partition LMA into LMAm and LMAs for each leaf sample), we implemented
tests \DIFdelbegin \DIFdel{with
}\DIFdelend \DIFaddbegin \DIFadd{using }\DIFaddend randomized data to assess potential overfitting.
\DIFaddbegin 

\DIFaddend We generated 10 different randomized datasets by randomizing all trait
values (LMA, \emph{A}\textsubscript{area}, \emph{R}\textsubscript{area}
and LL) across \DIFdelbegin \DIFdel{species. Thus}\DIFdelend \DIFaddbegin \DIFadd{leaf samples, and then we fitted the best models (Table
1) to these randomized datasets. As a result}\DIFaddend , the randomized datasets
had zero expected covariance among traits.
\DIFdelbegin \DIFdel{Models fit }\DIFdelend \DIFaddbegin 

\DIFadd{Models fitted }\DIFaddend to the randomized datasets either did not converge or
showed divergent transitions (\DIFdelbegin \DIFdel{Supplementary Section
3}\DIFdelend \DIFaddbegin \DIFadd{Table AS1, Figure AS1-2}\DIFaddend ), indicating that
...
\citeproc{ref-Betancourt2016}{Betancourt M 2016}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ). Furthermore, when
models were \DIFdelbegin \DIFdel{fit }\DIFdelend \DIFaddbegin \DIFadd{fitted }\DIFaddend to randomized data, the scaling exponents
(\(\alpha_{m,s}\), \(\beta_{m,s}\), and \(\gamma_{m,s}\)) were not
significantly different from zero. In simple terms, the two-dimensional
models failed when \DIFdelbegin \DIFdel{fit }\DIFdelend \DIFaddbegin \DIFadd{fitted }\DIFaddend to randomized data.
\DIFaddbegin 
...
\DIFdelbegin %DIFDELCMD < \includegraphics{../figs/coef_sim_gl.png}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \includegraphics{../figs/coef_sim_gl.pdf}
\DIFaddend 
...
Figure AS2 \DIFdelbegin \DIFdel{showsWe also checked }\DIFdelend \DIFaddbegin \DIFadd{shows }\DIFaddend the regression coefficients for the randomized Panama
dataset (\(\alpha_{0, m}\), \(\beta_{0, s}\), \(\gamma_{0, m, s}\), and
\(\theta\) \DIFdelbegin \DIFdel{in Eqs.2-4 }\DIFdelend \DIFaddbegin \DIFadd{as in Eqs. 2-4 }\DIFaddend in the main text). Details as for Figure AS1.
...
\DIFaddbegin \DIFadd{and }\DIFaddend did not show any patterns. Again, these tests with randomized data
indicate that our model is not inherently prone to overfitting or to
producing patterns from noise.

\DIFdelbegin %DIFDELCMD < \includegraphics{../figs/coef_sim_pa.png}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \includegraphics{../figs/coef_sim_pa.pdf}
\DIFaddend 

\newpage

\section{\DIFdelbegin \DIFdel{Supplementary Section 4: Understanding relationships between
photosynthetic capacity and
LMA}\DIFdelend \DIFaddbegin \DIFadd{Variance partitioning}\DIFaddend }\DIFdelbegin %DIFDELCMD < \label{supplementary-section-4-understanding-relationships-between-photosynthetic-capacity-and-lma}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{variance-partitioning}
...
\begin{equation}\DIFadd{\phantomsection\label{eq-vpart}{
...
\section{\DIFadd{Understanding and simulating relationships between
photosynthetic capacity and
...
\DIFdelbegin \DIFdel{relationships between }\emph{\DIFdel{A}}%DIFAUXCMD
\DIFdel{\textsubscript{max} and LMA }\DIFdelend \DIFaddbegin \DIFadd{these relationships }\DIFaddend are controversial
(\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\emph{A}\textsubscript{max} is often mass-normalized (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
;
\mbox{%DIFAUXCMD
\citeproc{ref-Shipley2006}{Shipley B et al. 2006}}\hspace{0pt}%DIFAUXCMD
;
\mbox{%DIFAUXCMD
\citeproc{ref-Blonder2011}{Blonder B et al. 2011}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ), it has been argued
that \emph{A}\textsubscript{max} should be area-normalized when
exploring trait relationships \DIFdelbegin \DIFdel{, }\DIFdelend because photosynthesis is an area-based
process (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
}\DIFaddend ). Consistent with
this argument, Osnas \DIFaddbegin \DIFadd{JLD }\DIFaddend et al. (\citeproc{ref-Osnas2013}{2013}) showed
...
area). Osnas \DIFaddbegin \DIFadd{JLD }\DIFaddend et al. (\citeproc{ref-Osnas2018}{2018}) further
\DIFdelbegin \DIFdel{showed }\DIFdelend \DIFaddbegin \DIFadd{demonstrated }\DIFaddend that the relationship between \emph{A}\textsubscript{max}
...
the following cases (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
Therefore, we used a multivariate normal distribution with }\emph{\DIFdel{r}} %DIFAUXCMD
\DIFdel{=
...
%DIFDELCMD < \label{supplementary-section-5-simulated-datasets-to-study-relationships-between-photosynthetic-capacity-and-lma}
%DIFDELCMD < 
...
distributions}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Osnas2018}{Osnas JLD et al. 2018}}\hspace{0pt}%DIFAUXCMD
)}\DIFaddend .
...
For each simulated \DIFdelbegin \DIFdel{dataset, we quantified the mass-dependence (}\emph{\DIFdel{b}}%DIFAUXCMD
\DIFdel{) using the ordinary least squares regression (log-log)form of Eq. 5. We repeated these steps }\DIFdelend \DIFaddbegin \DIFadd{set of LMAm and LMAs values, we generated
}\emph{\DIFadd{A}}\DIFadd{\textsubscript{area} values using the estimated values of
\(\alpha_m\) and \(\alpha_s\) (Eq. 2) from the corresponding
best-fitting model (GLOPNET or Panama); i.e., the model with the lowest
LOOIC. We generated }\DIFaddend 1000 \DIFdelbegin \DIFdel{times for each set of parameter values}\DIFdelend \DIFaddbegin \DIFadd{simulated datasets for GLOPNET, Panama sun
leaves, and Panama shade leaves, with each dataset having a sample size
of 100 leaves. For each simulated dataset, we used Eq.~\ref{eq-mass} to
quantify the relationship between }\emph{\DIFadd{A}}\DIFadd{\textsubscript{area} and LMA}\DIFaddend .

We also used Eqs. S5\DIFdelbegin \DIFdel{.1-S5}\DIFdelend .2\DIFaddbegin \DIFadd{-S5.4 }\DIFaddend to create the simulated LMA datasets in Fig.
1, which is based on our analysis of GLOPNET data.
\emph{A}\textsubscript{area} values were generated using Eq. 2 and
Eq.\DIFdelbegin \DIFdel{S1}\DIFdelend \DIFaddbegin \DIFadd{~\ref{eq-mass}}\DIFaddend . Based on our GLOPNET results, the standard deviations
...
\DIFdelbegin \section{\DIFdel{Supplementary Section 6: Variance
partitioning}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{supplementary-section-6-variance-partitioning}
%DIFDELCMD < 
...
\section{\DIFdelbegin \DIFdel{Figures}\DIFdelend \DIFaddbegin \DIFadd{Stan code}\DIFaddend }\DIFdelbegin %DIFDELCMD < \label{figures}
%DIFDELCMD < 

%DIFDELCMD < \begin{suppfig}
%DIFDELCMD < 

%DIFDELCMD < \centering{
%DIFDELCMD < 

%DIFDELCMD < \includegraphics{../figs/box_inter.png}
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{suppfig-box_inter}%%%
...
%DIFDELCMD < \includegraphics{../figs/box_frac_de.png}
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{suppfig-box_frac_de}%%%
\DIFdel{Boxplots comparing posterior medians
of the latent variable }\emph{\DIFdel{f}} %DIFAUXCMD
...
leaves were available). Note that LMAm = }\emph{\DIFdel{f}} %DIFAUXCMD
\DIFdel{\(\times\) LMA, and
LMAs = (1 -- }\emph{\DIFdel{f}}%DIFAUXCMD
...
%DIFDELCMD < \includegraphics{../figs/box_frac_pa.png}
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{suppfig-box_frac_pa}%%%
\DIFdel{Boxplots comparing posterior medians
of the latent variable }\emph{\DIFdel{f}} %DIFAUXCMD
\DIFdel{(the fraction of total LMA comprised by
...
%DIFDELCMD < \includegraphics{../figs/mass_prop_sim.png}
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{suppfig-mass_prop_sim}%%%
\DIFdel{The relationships between mass
dependency of }\emph{\DIFdel{A}}%DIFAUXCMD
...
indicate 95\% confidence intervals. Empirical estimates of }\emph{\DIFdel{b}} %DIFAUXCMD
\DIFdel{are
...
\textgreater{} }\emph{\DIFdel{b}} %DIFAUXCMD
...
%DIFDELCMD < \includegraphics{../figs/mass_prop_comp.png}
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{suppfig-mass_prop_comp}%%%
\DIFdel{The relationships between mass
dependency of }\emph{\DIFdel{A}}%DIFAUXCMD
\DIFdel{\textsubscript{max} (}\emph{\DIFdel{b}} %DIFAUXCMD
\DIFdel{in Eq. 5 in the main
...
%DIFDELCMD < \includegraphics{../figs/gl_point_np2.png}
%DIFDELCMD < 
...
%DIFDELCMD < \includegraphics{../figs/pa_point_npc_par.png}
%DIFDELCMD < 
...
%DIFDELCMD < \includegraphics{../figs/ps_point.png}
%DIFDELCMD < 
...
code}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{supplementary-section-7-stan-code}
%DIFDELCMD < %%%
...
\addcontentsline{toc}{section}{\DIFadd{Figures}}
...
\caption{\label{suppfig-box_frac_de}\DIFadd{Boxplots comparing posterior medians
...
LMAs = (1 -- }\emph{\DIFadd{f}}\DIFadd{) \(\times\) LMA. Boxplot symbols as in Fig. 5.
Groups sharing the same letters are not significantly different (P
\textgreater{} 0.05; t-tests). Estiamtes are from the best GLOPNET and
Panama models (Table 1).}}
...
samples in the Panama dataset (species for which both sun and shade
leaves were available). Note that LMAm = }\emph{\DIFadd{f}} \DIFadd{\(\times\) LMA, and
LMAs = (1 -- }\emph{\DIFadd{f}}\DIFadd{) \(\times\) LMA. Boxplot symbols as in Fig. 6.
Groups sharing the same letters are not significantly different (P
\textgreater{} 0.05; t-tests). Estiamtes are from the best GLOPNET and
Panama models (Table 1).}}
...
\caption{\label{suppfig-vpart}\DIFadd{Variance partitioning on LMA components
...
typically between 0 and 1 (\mbox{%DIFAUXCMD
\citeproc{ref-Osnas2018}{Osnas JLD et al.
2018}}\hspace{0pt}%DIFAUXCMD
...
\citeproc{ref-Osnas2018}{Osnas
JLD et al. 2018}}\hspace{0pt}%DIFAUXCMD
). Parameter values are based on the best GLOPNET model
...
\caption{\label{suppfig-mass_prop_comp}\DIFadd{The relationships between mass
dependency of }\emph{\DIFadd{A}}\DIFadd{\textsubscript{max} (}\emph{\DIFadd{b}} \DIFadd{in Eq. 5 in the main
text) and LMAs variance (relative to total LMA variance; Eq. 6) for
simulated datasets generated from a normal distribution (N) vs.~a
multivariate normal distribution (MVN). Parameter values are based on
...
\caption{\label{suppfig-gl_point_np2}\DIFadd{Measured traits related to
photosynthesis and metabolism (nitrogen and phosphorus per-unit leaf
...
\caption{\label{suppfig-pa_npc}\DIFadd{Measured traits in the Panama dataset
related to photosynthesis and metabolism (nitrogen and phosphorus
per-unit leaf area; }\emph{\DIFadd{N}}\DIFadd{\textsubscript{area} and
}\emph{\DIFadd{P}}\DIFadd{\textsubscript{area}) are better correlated with estimates
...
CL\textsubscript{area}). }\emph{\DIFadd{N}}\DIFadd{\textsubscript{area},
}\emph{\DIFadd{P}}\DIFadd{\textsubscript{area}, and CL\textsubscript{area} data were not
used to fit the models, and are presented here as independent support
for the model results. Pearson correlation coefficients (}\emph{\DIFadd{r}}\DIFadd{) for
...
obtained for }\emph{\DIFadd{N}}\DIFadd{\textsubscript{area} and
}\emph{\DIFadd{P}}\DIFadd{\textsubscript{area} for GLOPNET (Fig. S1). The results shown
here include all leaves for which }\emph{\DIFadd{N}}\DIFadd{\textsubscript{area},
}\emph{\DIFadd{P}}\DIFadd{\textsubscript{area} and CL\textsubscript{area} are available.}}
...
\caption{\label{suppfig-pa_point_npc_par}\DIFadd{Partial regression plots for
nitrogen, phosphorus and cellulose per-unit leaf area
...
\caption{\label{suppfig-LMAm_LMAs}\DIFadd{Pearson correlation coefficients for
posterior medians of LMAm vs LMAs in the (a) GLOPNET and (b) Panama
...
\DIFaddend \bibitem[\citeproctext]{ref-Alvarez2014}
Alvarez \DIFdelbegin \DIFdel{, I., Niemi , J., Simpson , M ., 2014. }\DIFdelend \DIFaddbegin \DIFadd{I, Niemi J, Simpson M (2014) }\DIFaddend Bayesian inference for a covariance
matrix. arXiv preprint arXiv:1408.4050v2\DIFaddbegin \DIFadd{:}\DIFaddend 1--12.
\url{https://doi.org/10.1214/aos/1176348885}\DIFaddbegin \DIFadd{.
}\DIFaddend 

\bibitem[\citeproctext]{ref-Betancourt2016}
Betancourt \DIFdelbegin \DIFdel{, M ., 2016. }\DIFdelend \DIFaddbegin \DIFadd{M (2016) }\DIFaddend Diagnosing {Suboptimal Cotangent Disintegrations} in
{Hamiltonian Monte Carlo}. arXiv.
\url{https://doi.org/10.48550/arXiv.1604.00695}\DIFaddbegin \DIFadd{.
}\DIFaddend 

\bibitem[\citeproctext]{ref-Blonder2011}
Blonder \DIFdelbegin \DIFdel{, B., Violle , C., Bentley , L.P., Enquist , B.J., 2011. }\DIFdelend \DIFaddbegin \DIFadd{B, Violle C, Bentley LP, Enquist BJ (2011) }\DIFaddend Venation networks and
the origin of the leaf economics spectrum. Ecology \DIFdelbegin \DIFdel{Letters
14, }\DIFdelend \DIFaddbegin \DIFadd{Letters14:}\DIFaddend 91--100.
\url{https://doi.org/10.1111/j.1461-0248.2010.01554.x}\DIFaddbegin \DIFadd{.
}\DIFaddend 

\bibitem[\citeproctext]{ref-Gelman2008}
Gelman \DIFdelbegin \DIFdel{, A., Jakulin , A., Pittau , M.G., Su , Y.S., 2008. }\DIFdelend \DIFaddbegin \DIFadd{A, Jakulin A, Pittau MG, Su YS (2008) }\DIFaddend A weakly informative
default prior distribution for logistic and other regression models.
Annals of Applied \DIFdelbegin \DIFdel{Statistics 2, }\DIFdelend \DIFaddbegin \DIFadd{Statistics2:}\DIFaddend 1360--1383.
\url{https://doi.org/10.1214/08-AOAS191}\DIFaddbegin \DIFadd{.
}\DIFaddend 
...
Lewandowski \DIFdelbegin \DIFdel{, D., Kurowicka , D., Joe , H ., 2009. }\DIFdelend \DIFaddbegin \DIFadd{D, Kurowicka D, Joe H (2009) }\DIFaddend Generating random correlation
matrices based on vines and extended onion method. Journal of
Multivariate \DIFdelbegin \DIFdel{Analysis 100, }\DIFdelend \DIFaddbegin \DIFadd{Analysis100:}\DIFaddend 1989--2001.
\url{https://doi.org/10.1016/j.jmva.2009.04.008}\DIFaddbegin \DIFadd{.
}\DIFaddend 
...
Shipley \DIFdelbegin \DIFdel{, B., Lechowicz , M.J., Wright , I., Reich , P.B., 2006. }\DIFdelend \DIFaddbegin \DIFadd{B, Lechowicz MJ, Wright I, Reich PB (2006) }\DIFaddend Fundamental
trade-offs generating the worldwide leaf economics spectrum.
\DIFdelbegin \DIFdel{Ecology 87,
}\DIFdelend \DIFaddbegin \DIFadd{Ecology87:}\DIFaddend 535--541. \url{https://doi.org/10.1890/05-1051}\DIFaddbegin \DIFadd{.
}\DIFaddend 
...
E, Groom PK, Gulias J, Hikosaka K, Lamont BB, Lee T, Lee W, Lusk C}\DIFaddend ,
\DIFaddbegin \DIFadd{Midgley JJ, Navas ML, Niinemets }\DIFaddend Ü\DIFdelbegin \DIFdel{., Oleksyn , J., Osada , H., Poorter , H., Pool
