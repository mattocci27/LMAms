...
%DIF ADD ms/LMAms_main.tex       Mon Dec 20 18:02:44 2021
...
\DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{(6a) (}\DIFaddend no light effects)\DIFdelbegin \DIFdel{for GL}\DIFdelend , because GLOPNET \DIFdelbegin \DIFdel{includes no information on light availability}\DIFdelend \DIFaddbegin \DIFadd{does not report canopy position and thus primarily represents interspecific variation (see above)}\DIFaddend .
To model LL for Panama, we used Eq. \DIFdelbegin %DIFDELCMD < \eqref{eq:E-LL2} %%%
...
Specifically, we }\DIFdelend \DIFaddbegin \DIFadd{), we implemented tests with randomized data (}\emph{\DIFadd{n}} \DIFadd{= 10) to assess potential overfitting.
...
We therefore }\DIFaddend assume that estimates of LMAp and LMAs \DIFaddbegin \DIFadd{obtained from the GLOPNET and Panama datasets }\DIFaddend reflect meaningful patterns in the \DIFdelbegin \DIFdel{observed data}\DIFdelend \DIFaddbegin \DIFadd{observations and allow for meaningful tests of our hypotheses}\DIFaddend .

\DIFdelbegin \DIFdel{To assess if decomposing LMA into photosynthetic and structural components lead to improved our model}\DIFdelend \DIFaddbegin \DIFadd{Instead of using exact leave-one-out cross-validation (LOO), which is a robust way to compare models with different numbers of parameters but computationally impractical}\DIFaddend , we compared \DIFdelbegin \DIFdel{our model with models that only use LMA instead of LMAp and LMAs.
We assessed the model performance by }\DIFdelend \DIFaddbegin \DIFadd{the performance of different models (Table 1) }\DIFaddend using Pareto-smoothed importance sampling leave-one-out cross-validation (\DIFdelbegin \DIFdel{LOO}\DIFdelend \DIFaddbegin \DIFadd{PSIS-LOO}\DIFaddend ; Vehtari et al. (\protect\hyperlink{ref-Vehtari2014}{2014})\DIFdelbegin \DIFdel{).
...
\DIFdelend \DIFaddbegin \DIFadd{8 }\DIFaddend uses area-normalized \emph{A}\textsubscript{max}, equivalent results \DIFdelbegin \DIFdel{can be }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend obtained from mass-normalized \emph{A}\textsubscript{max} (\DIFdelbegin \DIFdel{Osnas et al. }\DIFdelend \protect\DIFdelbegin %DIFDELCMD < \hyperlink{ref-Osnas2018}{2018}%%%
\DIFdelend \DIFaddbegin \hyperlink{ref-Osnas2018}{Osnas et al. 2018}\DIFaddend ).
...
\DIFdel{\textsubscript{area} to LMAp and LMAs in Eq }%DIFDELCMD < \eqref{eq:E-A}%%%
\DIFdel{), as well as the }\DIFdelend \DIFaddbegin \DIFadd{prepared LMAp values based on the }\DIFaddend empirical estimates of these values \DIFdelbegin \DIFdel{based on }\DIFdelend \DIFaddbegin \DIFadd{from our }\DIFaddend GLOPNET and Panama \DIFdelbegin \DIFdel{datasets.
Each simulation had a sample size of 100 leaves, where the values of LMAp and LMAs for each leaf was }\DIFdelend \DIFaddbegin \DIFadd{analyses.
LMAs was generated by }\DIFaddend a random draw from a lognormal distribution with \DIFdelbegin \DIFdel{median and variance as follows: for simulations (i)-(iii), median log(40) and variance log(1) or log(4)(for low and high variance, respectively); and for simulations (iv)-(vi), medians and variances were the posterior means from our GLOPNET and Panama analyses}\DIFdelend \DIFaddbegin \DIFadd{the empirical estimates of mean values, while with changing the standard deviation of log(LMAs) between log(1.01) and log(10)}\DIFaddend .
...
%DIFAUXCMD
\DIFdel{Metabolic leaf mass (LMAp) accounted for nearly all leaf dark respiration; i.e., estimated dark respiration rate per-unit structural mass (\(\gamma_s\)) was close to zero in analyses of both GLOPNET and Panama data (Table \ref{tab:paratab}).
Thus, although building costs are likely similar for different leaf chemical components and tissues (Williams et al.
}%DIFDELCMD < \protect\hyperlink{ref-Williams1989}{1989}%%%
\DIFdel{, Villar and Merino }%DIFDELCMD < \protect\hyperlink{ref-Villar2001}{2001}%%%
\DIFdel{), our results suggest that leaf mass associated with photosynthetic function accounts for nearly all leaf maintenance respiration}\DIFdelend \DIFaddbegin \emph{\DIFadd{A}}\DIFadd{\textsubscript{area} was then generated based on LMAp, LMAs and \(\alpha\) values (Eq. 4).
Using the above values, we calculated }\emph{\DIFadd{b}} \DIFadd{for GLOPNET, Panama sun leaves and Panama shade leaves.
Each simulation had a sample size of 100 leaves, and we repeated these steps 1000 times}\DIFaddend .
...
Burnham, K. P., and D. R. Anderson. 2002. \href{https://doi.org/10.1007/b97636}{Model {Selection} and {Multimodel Inference}: {A Practical \DIFdelbegin \DIFdel{Information}%DIFDELCMD < }%%%
\DIFdel{-}%DIFDELCMD < {%%%
...
Westoby, M., P. B. Reich, and I. J. Wright. 2013. \href{https://doi.org/10.1111/nph.12345}{Understanding ecological variation across species: {\DIFdelbegin \DIFdel{Area}\DIFdelend \DIFaddbegin \DIFadd{Area-based}\DIFaddend } \DIFdelbegin \DIFdel{-based }\DIFdelend vs mass-based expression of leaf traits}. New Phytologist 199:322--323.
...
(1) LMA,
(2) }\DIFaddend LMAp and LMAs\DIFdelbegin %DIFDELCMD < & %%%
...
\DIFdel{106 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{LMAp+ }\DIFdelend \DIFaddbegin \DIFadd{,
(3) LMA and light,
(4) LMAp, LMAs and light, and
(5) LMAp, }\DIFaddend LMAs/LT \DIFdelbegin \DIFdel{+ light}%DIFDELCMD < & %%%
...
%DIFDELCMD < \end{longtable}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{and light, with four types of parameter restrictions:
(a) \(\alpha_s = 0\) and \(\beta_p = 0\),
(b) \(\alpha_p > \alpha_s\) and \(\beta_p = 0\),
(c) \(\alpha_s = 0\) and \(\beta_s > \beta_p\), and
