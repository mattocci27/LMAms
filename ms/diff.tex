...
%DIF DEL LMAms_main_old.tex   Thu Aug 26 00:41:36 2021
%DIF ADD LMAms_main.tex       Thu Aug 26 00:43:47 2021
...
These two independent axes can be translated into either a two-dimensional trait space (if metabolic traits are area-normalized; Fig. \DIFaddbegin \DIFadd{Fig.~}\DIFaddend \ref{fig:Hplt}b) or a one-dimensional trait space (if metabolic traits are mass-normalized; Fig. \DIFaddbegin \DIFadd{Fig.~}\DIFaddend \ref{fig:Hplt}c).
...
\ref{fig:Hplt}bc).
}\DIFdelend \DIFaddbegin \DIFadd{While both Fig. Fig.~\ref{fig:Hplt}b and Fig. Fig.~\ref{fig:Hplt}c are both `correct' representations of the same data, they lead to different perceptions about the dimensionality of leaf functional variation.
...
The hypothetical example in Fig. \DIFaddbegin \DIFadd{Fig.~}\DIFaddend \ref{fig:Hplt} shows how mass-normalization can, in principle, make a two-dimensional trait space appear one-dimensional, but this example does not resolve the dimensionality of functional variation in real leaf assemblages.
...
We \DIFdelbegin \DIFdel{developed a statistical modeling framework to partition LMAinto additive LMAs and LMAm components (see below and Supplement SX).
For sample }\emph{\DIFdel{i}} %DIFAUXCMD
\DIFdel{(e.g.
, sun or shade leaves of a given species; see Datasets below) , we partition LMA \textsubscript{\emph{i}} into LMAm\textsubscript{\emph{i}} = }\emph{\DIFdel{f\textsubscript{i}}} %DIFAUXCMD
...
\DIFdel{(high LMAm\textsubscript{\emph{i}})for leaves with high }\emph{\DIFdel{A}}%DIFAUXCMD
...
We considered five sets of models \DIFaddbegin \DIFadd{(summarized in Table \ref{tab:modeltab})}\DIFaddend , ranging from simple models with LMA as the sole predictor, to more complex models in which LMA was partitioned into metabolic and structural components (\DIFdelbegin \DIFdel{LMAm }\DIFdelend \DIFaddbegin \DIFadd{LMAp }\DIFaddend and LMAs).
...
Next, we considered \DIFaddbegin \DIFadd{several }\DIFaddend models in which LMA is partitioned into additive metabolic and structural functions; i.e., we assume that the sum of metabolic leaf mass per area (\DIFdelbegin \DIFdel{LMAm}\DIFdelend \DIFaddbegin \DIFadd{LMAp}\DIFaddend ) and structural leaf mass per area (LMAs) is equal to total observed LMA for leaf sample \emph{i}\DIFdelbegin \DIFdel{(where a `sample' is a species in the GLOPNET dataset, or a species × canopy position combination in the Panama dataset)}\DIFdelend :

\begin{align}
  &\mathrm{LMA}_{i} =\DIFdelbegin \DIFdel{\mathrm{LMAm}}\DIFdelend \DIFaddbegin \DIFadd{\mathrm{LMAp}}\DIFaddend _{i} + \mathrm{LMAs}_{i} \label{eq:LMA}\\
  &\DIFdelbegin \DIFdel{\mathrm{LMAm}}\DIFdelend \DIFaddbegin \DIFadd{\mathrm{LMAp}}\DIFaddend _{i} = f_{i} \mathrm{LMA}_{i} \DIFdelbegin %DIFDELCMD < \label{eq:LMAm}%%%
\DIFdelend \DIFaddbegin \label{eq:LMAp}\DIFaddend \\
  &\mathrm{LMAs}_{i} = (1 - f_{i})  \mathrm{LMA}_{i}\label{eq:LMAs}
\end{align}

where \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend \emph{f\textsubscript{i}} \DIFdelbegin \DIFdel{is the fraction of LMA \textsubscript{\emph{i}} that is comprised of LMAm\textsubscript{\emph{i}}.
The }\DIFdelend \DIFaddbegin \DIFadd{values -- the fractions of LMA comprised of LMAp for each sample }\DIFaddend \emph{\DIFdelbegin \DIFdel{f\textsubscript{i}}\DIFdelend \DIFaddbegin \DIFadd{i}\DIFaddend } \DIFdelbegin \DIFdel{terms are not directly observed but can be }\DIFdelend \DIFaddbegin \DIFadd{-- are }\DIFaddend estimated as latent variables in \DIFdelbegin \DIFdel{a Bayesian }\DIFdelend \DIFaddbegin \DIFadd{our }\DIFaddend modeling framework (see details below).
...
\DIFdel{\textsubscript{area}) is determined by LMAm, LL is determined by LMAs , and total leaf dark respiration is determined by both metabolic and structural tissues}\DIFdelend \DIFaddbegin \DIFadd{The simplest of these forms is a multivariate power-law}\DIFaddend :
...
\DIFdel{\textsubscript{area}, respectively, to variation in LMAm; \(\alpha_s\), \(\beta_s\), and \(\gamma_s\) are fitted exponents that quantify the sensitivities of }\emph{\DIFdel{A}}%DIFAUXCMD
\DIFdel{\textsubscript{area}, LL and }\emph{\DIFdel{R}}%DIFAUXCMD
...
\DIFdelend \DIFaddbegin \DIFadd{(5) }\DIFaddend was replaced by leaf structural density \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend LMAs/LT, where LT is leaf thickness\DIFdelbegin \DIFdel{.
...
%DIFDELCMD < %%%
\begin{align*}
\DIFdel{\mathrm{E[LL}_i}] \DIFdel{= \beta_0\mathrm{LMAm}_{i}^{\beta_m} \mathrm{LMAs}_{i}^{\beta_s} exp(\theta Light_i) %DIFDELCMD < \label{eq:E-LL2}%%%
}\end{align*}%DIFAUXCMD
%DIFDELCMD < 
...
Prior distributions for the latent variables \emph{f}\textsubscript{i} (which are used to partition LMA into \DIFdelbegin \DIFdel{LMAm }\DIFdelend \DIFaddbegin \DIFadd{LMAp }\DIFaddend and LMAs according to Eqs. \eqref{eq:E-A}-\eqref{eq:E-R}) were non-informative uniform\DIFdelbegin \DIFdel{priors beta(1}\DIFdelend \DIFaddbegin \DIFadd{(0}\DIFaddend , 1) \DIFaddbegin \DIFadd{distributions, }\DIFaddend (i.e., LMA was partitioned based on patterns in the data).
See supplement X for more detail.
The Stan code use to fit models \DIFdelbegin \DIFdel{are also }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend available from Github at: \href{https://github.com/mattocci27/LMAmLMAs}{https://github.com/mattocci27/XXX}.

\DIFdelbegin \DIFdel{Some parameters are obviously exchangeable in our model }\DIFdelend \DIFaddbegin \DIFadd{In the absence of additional constraints, some parameters are exchangeable (and the model is therefore non-identifiable), because different parameter values can lead to the identical expected values of }\emph{\DIFadd{A}}\DIFadd{\textsubscript{area}, }\emph{\DIFadd{R}}\DIFadd{\textsubscript{area}, and LL}\DIFaddend .
For example, \DIFdelbegin \DIFdel{\((f_i, \alpha_m, \alpha_s)\) }\DIFdelend \DIFaddbegin \DIFadd{\((f_i, \alpha_p, \alpha_s)\) }\DIFaddend = (0.2, 0.5, 1.0) and (0.8, 1.0, 0.5) yield the \DIFdelbegin \DIFdel{invariance of the likelihood}\DIFdelend \DIFaddbegin \DIFadd{identical expectation for }\emph{\DIFadd{A}}\DIFadd{\textsubscript{area}}\DIFaddend .
To avoid this non-identifiability, we explore two methods based on our \DIFdelbegin \DIFdel{assumption }\DIFdelend \DIFaddbegin \DIFadd{biological assumption (following Osnas et al. (}\protect\hyperlink{ref-Osnas2018}{2018}\DIFadd{)) }\DIFaddend that \emph{A}\textsubscript{area} depends more strongly on metabolic leaf mass (\DIFdelbegin \DIFdel{LMAm; parameter \(\alpha_m\) }\DIFdelend \DIFaddbegin \DIFadd{LMAp; parameter \(\alpha_p\) }\DIFaddend in Eq. \eqref{eq:E-A}) than structural leaf mass (LMAs; parameter \(\alpha_s\) in \DIFdelbegin \DIFdel{Eq}\DIFdelend \DIFaddbegin \DIFadd{Eqs}\DIFaddend . \eqref{eq:E-A}.
In the first method, we switched the \DIFdelbegin \DIFdel{\(\alpha_m\) }\DIFdelend \DIFaddbegin \DIFadd{\(\alpha_p\) }\DIFaddend and \(\alpha_s\) labels within a chain when the posterior means for \DIFdelbegin \DIFdel{\(\alpha_m\) }\DIFdelend \DIFaddbegin \DIFadd{\(\alpha_p\) }\DIFaddend was smaller than that for \(\alpha_s\) after the posterior distributions converged.
In the second method, we constrained \DIFdelbegin \DIFdel{\(\alpha_m\) }\DIFdelend \DIFaddbegin \DIFadd{\(\alpha_p\) }\DIFaddend \textgreater{} \(\alpha_s\) during MCMC.
Both methods yielded qualitatively similar results; we report results from the second method.
\DIFaddbegin \DIFadd{Constraing \(\alpha_p\) \textgreater{} \(\alpha_s\) alone also constrains posterior distributions of the ohter parameters, and thus we did not}\\
\DIFaddend Convergence of the posterior distribution was assessed with the Gelman-Rubin statistic with a convergence threshold of 1.1 for all \DIFdelbegin \DIFdel{diagnostics (Gelman et al. }\DIFdelend \DIFaddbegin \DIFadd{parameters (}\DIFaddend \protect\DIFdelbegin %DIFDELCMD < \hyperlink{ref-Gelman2013}{2013}%%%
\DIFdelend \DIFaddbegin \hyperlink{ref-Gelman2013}{Gelman et al. 2013}\DIFaddend ).

Because our statistical approach includes \DIFaddbegin \DIFadd{many parameters (}\DIFaddend one latent variable \emph{f\textsubscript{i}} to partition LMA into LMAm and LMAs for each leaf sample\DIFdelbegin \DIFdel{, one might expect a good match between predictions and observations simply due to the large number of free parameters, whether or not the model captured important biological mechanisms.
...
\DIFaddbegin \DIFadd{In contrast, models fit to observed data converged and produced significant results for both GLOPNET and Panama (see Results).
}\DIFaddend Thus, the \DIFaddbegin \DIFadd{tests with randomized data indicate that our }\DIFaddend model is not \DIFdelbegin \DIFdel{guaranteed to work based on the assumptions alone.
We }\DIFdelend \DIFaddbegin \DIFadd{inherently prone to overfitting or to producing patterns from noise.
We therefore }\DIFaddend assume that estimates of \DIFdelbegin \DIFdel{LMAm and LMAs }\DIFdelend \DIFaddbegin \DIFadd{LMAp and LMAs obtained from the GLOPNET and Panama datasets }\DIFaddend reflect meaningful patterns in the \DIFdelbegin \DIFdel{observed data.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{To assess if decomposing LMA into photosynthetic and structural components lead to improved our model, we compared our model with models that only use LMA instead of LMAm and LMAs.
...
We used the following identity to estimate the relative contributions of \DIFdelbegin \DIFdel{LMAm }\DIFdelend \DIFaddbegin \DIFadd{LMAp }\DIFaddend and LMAs to LMA variance, where again LMA = \DIFdelbegin \DIFdel{LMAm }\DIFdelend \DIFaddbegin \DIFadd{LMAp }\DIFaddend + LMAs:
...
}\DIFaddend Thus, the fractions of total LMA variance due to variance in \DIFdelbegin \DIFdel{LMAm and LMAs were determined by the covariances }\DIFdelend \DIFaddbegin \DIFadd{LMAp and LMAs are }\DIFaddend Cov(LMA, \DIFdelbegin \DIFdel{LMAm)}\DIFdelend \DIFaddbegin \DIFadd{LMAp)/Var(LMA) }\DIFaddend and Cov(LMA, LMAs)\DIFdelbegin \DIFdel{, respectively, taken as proportions of the total variance }\DIFdelend \DIFaddbegin \DIFadd{/}\DIFaddend Var(LMA)\DIFaddbegin \DIFadd{, respectively}\DIFaddend .
...
We applied our \DIFdelbegin \DIFdel{LMAm}\DIFdelend \DIFaddbegin \DIFadd{LMAp}\DIFaddend +LMAs model (Eq. \eqref{eq:E-A}) to simulated data to better understand relationships between photosynthetic capacity (\emph{A}\DIFdelbegin \DIFdel{\textsubscript{max}}\DIFdelend ) and LMA.
...
\DIFaddbegin \DIFadd{Osnas et al. (}\protect\hyperlink{ref-Osnas2018}{2018}\DIFadd{) further showed that this result (strong area-dependence but weak mass-dependence of A across species) is most apparent within species groups in which LL is highly variable.
}\DIFaddend 

To better understand the factors affecting \emph{A} \DIFdelbegin \DIFdel{\textsubscript{max} }\DIFdelend vs.~LMA relationships, we created simulated datasets in which we varied the following factors: the sensitivity of \emph{A}\textsubscript{area} to variation in \DIFdelbegin \DIFdel{LMAm (parameter \(\alpha_m\) }\DIFdelend \DIFaddbegin \DIFadd{LMAp (parameter \(\alpha_p\) }\DIFaddend in Eq. \eqref{eq:E-A}); the sensitivity of \emph{A}\textsubscript{area} to LMAs (parameter \(\alpha_s\) in Eq. \eqref{eq:E-A}), and the degree to which \DIFdelbegin \DIFdel{LMAm }\DIFdelend \DIFaddbegin \DIFadd{LMAp }\DIFaddend and LMAs varied among leaves.
...
where LMA is the sum of \DIFdelbegin \DIFdel{LMAm }\DIFdelend \DIFaddbegin \DIFadd{LMAp }\DIFaddend and LMAs (Eq. \eqref{eq:LMA}), \emph{a} is a fitted constant, and \emph{b} is an index of mass-dependence as illustrated by the following cases (\DIFdelbegin \DIFdel{Osnas et al. }\DIFdelend \protect\DIFdelbegin %DIFDELCMD < \hyperlink{ref-Osnas2013}{2013}%%%
...
We created simulated datasets based on a range of assumptions for the LMA variance components: (i) \DIFdelbegin \DIFdel{LMAm variance \textless\textless{} }\DIFdelend \DIFaddbegin \DIFadd{low LMAp variance and high }\DIFaddend LMAs variance, (ii) \DIFdelbegin \DIFdel{LMAm variance = }\DIFdelend \DIFaddbegin \DIFadd{low LMAp variance and low }\DIFaddend LMAs variance, (iii) \DIFdelbegin \DIFdel{LMAm variance \textgreater\textgreater{} }\DIFdelend \DIFaddbegin \DIFadd{high LMAp variance and low }\DIFaddend LMAs variance, (iv) our estimates (posterior means) of \DIFdelbegin \DIFdel{LMAm }\DIFdelend \DIFaddbegin \DIFadd{LMAp }\DIFaddend and LMAs variances from GLOPNET, (v) our estimates of \DIFdelbegin \DIFdel{LMAm }\DIFdelend \DIFaddbegin \DIFadd{LMAp }\DIFaddend and LMAs variances for Panama sun leaves, and (vi) our estimates of \DIFdelbegin \DIFdel{LMAm }\DIFdelend \DIFaddbegin \DIFadd{LMAp }\DIFaddend and LMAs variances for Panama shade leaves.
For each of the above cases, we considered a range of values for and (which determine the sensitivity of \emph{A}\textsubscript{area} to \DIFdelbegin \DIFdel{LMAm }\DIFdelend \DIFaddbegin \DIFadd{LMAp }\DIFaddend and LMAs in Eq \eqref{eq:E-A}), as well as the empirical estimates of these values based on GLOPNET and Panama datasets.
Each simulation had a sample size of 100 leaves, where the values of \DIFdelbegin \DIFdel{LMAm }\DIFdelend \DIFaddbegin \DIFadd{LMAp }\DIFaddend and LMAs for each leaf was a random draw from a lognormal distribution \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{wth }\DIFaddend median and variance as follows: for simulations (i)-(iii), median \DIFdelbegin \DIFdel{log(}\DIFdelend 40 \DIFdelbegin \DIFdel{) and variance log}\DIFdelend \DIFaddbegin \DIFadd{and standard deviaitons of exp}\DIFaddend (1) or \DIFdelbegin \DIFdel{log}\DIFdelend \DIFaddbegin \DIFadd{exp}\DIFaddend (4) (for low and high variance \DIFaddbegin \DIFadd{cases}\DIFaddend , respectively); and for simulations (iv)-(vi), medians and \DIFdelbegin \DIFdel{variances }\DIFdelend \DIFaddbegin \DIFadd{standard deviations }\DIFaddend were the posterior means from our GLOPNET and Panama analyses.
...
%DIFDELCMD < \caption{Example of a two-dimensional functional space that can result in either a two- or one-dimensional trait space, depending on how metabolic traits are normalized. (A) Hypothetical indpendent variation in two leaf mass per area (LMA) components: metabolic leaf mass per area (LMAm, which largely determines per-area values of photosynthesis, respiration, and nutrient concentrations) and structural leaf mass per area (LMAs, which determines leaf toughness but has little effect on metabolic traits).
...
%DIFDELCMD < (C) One-dimensional relationship between \emph{A}\textsubscript{max} per-unit leaf mass and total LMA. Variation in LMAm and LMAs \emph{A}\textsubscript{max} values in panels B and C are derived from our analysis of the GLOPNET database. \(A_{\mathrm{area} \, i}=1.9\mathrm{LMAm}_i^{0.36}\mathrm{LMAs}^{-0.25}\epsilon_i\), where \(\epsilon_i\) is log-normally distributed with log-mean with 0, log-scale parameter with 0.3 (see Methods and Results).''\}}\label{fig:Hplt}
%DIFDELCMD < }
%DIFDELCMD < %%%
