...
%DIF DEL LMAms_main_old.tex   Thu Aug 22 03:08:34 2024
%DIF ADD LMAms_main.tex       Thu Aug 22 03:08:10 2024
...
}\DIFaddend by leaf structural density (LMAs/LT, where LT is leaf thickness), which
is based on the observation that cellulose density is a good predictor
for LL in some species assemblages (\citeproc{ref-Kitajima2013}{Kitajima
et al., 2013}, \citeproc{ref-Kitajima2012}{2012}).
\DIFaddbegin 

\DIFaddend These models with leaf thickness yielded qualitatively similar results
as those without leaf thickness, but they did not perform as well in the
model evaluation (see below).

\DIFdelbegin \DIFdel{For example}\DIFdelend \DIFaddbegin \DIFadd{The LMAm-LMAs model framework (Eqs. 1-4) has multiple identical
...
patterns in the data). See Supplementary Section 1 for more details.
...
\DIFdel{. }\DIFdelend Convergence of the posterior distribution was assessed with the
Gelman-Rubin statistic with a convergence threshold of 1.05 for all
parameters (\citeproc{ref-Gelman2013}{Gelman et al., 2013}).
...
LMAm and LMAs through randomized tests, detailed in Supplementary
Section 3. These tests support our assumption that the patterns observed
in the GLOPNET and Panama datasets are meaningful and allow for a
substantial exploration of our questions.
...
(Fig.~\ref{fig-vpart}a-b). \DIFdelbegin \DIFdel{Despite the relatively large residuals in
LMAs (Fig.~\ref{fig-vpart}a-b), the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend higher total LMA in evergreen compared to
deciduous leaves was primarily due to differences in LMAs (Figs.
\ref{fig-box_de}, S1, and S2). In the Panama dataset, light (sun
vs.~shade) explained \DIFdelbegin \DIFdel{much }\DIFdelend more variation in LMAm than LMAs, whereas site (wet
vs.~dry) explained much more variation in LMAs than LMAm
(Fig.~\ref{fig-vpart}c). Panama wet-site leaves had higher total LMA
than dry-site leaves due to greater \DIFdelbegin \DIFdel{LMAs when we compared the same
canopy strata }\DIFdelend \DIFaddbegin \DIFadd{wet-site LMAs for both sun and shade
leaves }\DIFaddend (Figs. \ref{fig-box_pa} and S1). In contrast, Panama sun leaves
...
\ref{fig-box_pa} and S1). \DIFdelbegin \DIFdel{Thus, }\DIFdelend LMAs comprised a larger fraction of total LMA
...
}\DIFdelend (Fig.~\ref{fig-mass_prop}, Figs. S4-S5). For example, \DIFaddbegin \DIFadd{in
...
Fig.~\ref{fig-pa_npc}) \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{were primarily }\DIFaddend due to sun leaves having higher
