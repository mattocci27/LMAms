...
%DIF DEL LMAms_main_old.tex   Thu Feb 13 11:15:09 2025
%DIF ADD LMAms_main.tex       Thu Feb 13 11:14:48 2025
...
al. 2017}) and in carbon and nutrient cycling (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-Tcherkez2017}{Tcherkez et al. 2017}}\hspace{0pt}%DIFAUXCMD
; }\DIFdelend e.g.,
\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Tcherkez2017}{Tcherkez et al. 2017}}\hspace{0pt}%DIFAUXCMD
;
}\DIFaddend \citeproc{ref-Huntingford2017}{Huntingford et al. 2017}). Thus,
...
proposed by Osnas et al. (\citeproc{ref-Osnas2018}{2018}) could be
...
Panama dataset. \DIFaddbegin \DIFadd{Note that we did not fit models to
...
LMA (Figs. \ref{fig-gl_point}-\ref{fig-pa_point}). \DIFaddbegin \DIFadd{LMAm and LMAs were
...
decomposing LMA variation into separate }\DIFdelend \DIFaddbegin \DIFadd{By decomposing leaf mass per area (LMA) into }\DIFaddend metabolic and structural
components (LMAm and LMAs, respectively)\DIFdelbegin \DIFdel{leads to improved predictions
of photosynthetic capacity (}\emph{\DIFdel{A}}%DIFAUXCMD
\DIFdel{\textsubscript{max}), dark
respiration rate (}\emph{\DIFdel{R}}%DIFAUXCMD
\DIFdel{\textsubscript{dark}), and leaf lifespan (LL)
...
%DIFDELCMD < %%%
\DIFdel{Although LMAm and LMAs are model-inferred (rather than observed) traits
...
revealed patterns consistent with previous analyses of measured traits (summarized below). Secondly, although our modeling approach requires
many parameters to partition LMA into LMAmand LMAs (one latent variable
...
model-inferred estimates of LMAm and LMAs}\DIFdelend \DIFaddbegin \DIFadd{, we show that mass‐based trait
...
vs.~\textasciitilde40\% for deciduous species (Fig. S3), and \textasciitilde70\% for shade leaves vs.~\textasciitilde40\% for }\DIFdelend \DIFaddbegin \DIFadd{Our analysis shows that evergreen
species and shade leaves allocate a higher fraction of their LMA to
structural mass compared with deciduous species and }\DIFaddend sun leaves (\DIFdelbegin \DIFdel{Fig.
...
}\emph{\DIFadd{R}}\DIFadd{\textsubscript{area} increases with LMA from shade leaves to sun
(Figs. \ref{fig-pa_point}d-f), whereas }\emph{\DIFadd{R}}\DIFadd{\textsubscript{area} is
similar between evergreen and deciduous leaves despite higher evergreen
LMA (Figs. \ref{fig-pa_point}d-f}\DIFaddend ).
...
parameter \(\theta\) in Table 2). Thus,
}\DIFdelend \DIFaddbegin \DIFadd{),
suggesting that toughness alone does not determine LL. Consequently,
simply }\DIFaddend partitioning LMA into LMAm and LMAs \DIFdelbegin \DIFdel{is not sufficient to }\DIFdelend \DIFaddbegin \DIFadd{cannot fully }\DIFaddend explain the
widely observed \DIFdelbegin \DIFdel{counter-gradient,
whereby }\DIFdelend \DIFaddbegin \DIFadd{counter‐gradient in which }\DIFaddend LL increases with \DIFdelbegin \DIFdel{increasing }\DIFdelend LMA across
species \DIFdelbegin \DIFdel{but decreases with increasing LMA within species }\DIFdelend \DIFaddbegin \DIFadd{yet decreases with LMA }\DIFaddend from shade to sun
...
}\DIFdelend \DIFaddbegin \DIFadd{We discuss potential
drivers of this pattern in the next section.
}

\DIFadd{Finally, the clustering of unclassified species (not reported as
...
(\mbox{%DIFAUXCMD
\citeproc{ref-Onoda2011}{Onoda et al. 2011}}\hspace{0pt}%DIFAUXCMD
)}\DIFaddend , \DIFdelbegin \DIFdel{how optimal LL depends on
light availability (\mbox{%DIFAUXCMD
\citeproc{ref-Kikuzawa1991}{Kikuzawa 1991}}\hspace{0pt}%DIFAUXCMD
) − as
well as
variation in leaf toughness for a given value of LMAs }\DIFdelend \DIFaddbegin \DIFadd{which likely explain
their low LMAs}\DIFaddend . \DIFdelbegin \DIFdel{In a
...
on BCI, Panama, the
difference between shade and sun leaves
was greater
...
LMAs is equal, with some LMAscomponents (e.g. , cellulose) imparting
greater toughness and longevity than others }\DIFdelend \DIFaddbegin \DIFadd{Herbaceous taxa with rapid leaf turnover may have little
incentive to invest in structural leaf mass, whereas woody species often
rely on thicker‐walled tissues to maintain longevity under stress }\DIFaddend (e.g.,
\DIFdelbegin \DIFdel{lignin in secondary
...
%DIFDELCMD < %%%
\DIFdel{In summary, the dependencies of }\emph{\DIFdel{A}}%DIFAUXCMD
\DIFdel{\textsubscript{area},
}\emph{\DIFdel{R}}%DIFAUXCMD
\DIFdel{\textsubscript{area}, and LL on model-inferred values of LMAm
...
canopy positions as well as ecophysiological tradeoffs across edaphic-climate gradients}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-Diaz2016}{Díaz et al. 2016}}\hspace{0pt}%DIFAUXCMD
;
\mbox{%DIFAUXCMD
...
modeling leaf function across diverse plant forms}\DIFaddend .
...
also revealed complexitiesthat were not anticipated by Osnas et al. (\mbox{%DIFAUXCMD
\citeproc{ref-Osnas2018}{2018}}\hspace{0pt}%DIFAUXCMD
). For example, if
...
to our interspecific analysis \DIFdelbegin \DIFdel{suggesting }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend weak mass-dependence of
\emph{A}\textsubscript{max} \DIFdelbegin \DIFdel{within sun and shade leaves
(}\DIFdelend \DIFaddbegin \DIFadd{at a given light level;
}\DIFaddend Fig.~\ref{fig-mass_prop}), \DIFdelbegin \DIFdel{estimates of intraspecific analysis of the
same Panama dataset suggests }\DIFdelend \DIFaddbegin \DIFadd{intraspecific patterns reveal }\DIFaddend strong
...
(\citeproc{ref-Moorcroft2001}{Moorcroft et al. 2001};
\citeproc{ref-Bonan2002}{Bonan et al. 2002};
...
structural leaf nitrogen components (\citeproc{ref-Dong2017}{Dong et al.
2017}, \citeproc{ref-Dong2022}{2022}), with over 20\% of total leaf
...
(\citeproc{ref-Hidaka2011}{Hidaka, and Kitayama 2011}).
\DIFdelbegin \DIFdel{We suggest that
future biogeochemical model developments }\DIFdelend \DIFaddbegin 

\DIFadd{Future biogeochemical model development should }\DIFaddend consider multiple axes of
leaf functional diversity, and \DIFdelbegin \DIFdel{that }\DIFdelend models with coupled carbon-nutrient
cycles \DIFaddbegin \DIFadd{should }\DIFaddend consider how LMA, nitrogen, and phosphorus are allocated
to different functions\DIFaddbegin \DIFadd{. Single-axis approaches will not capture the
...
(with minor implications for }\emph{\DIFadd{A}}\DIFadd{\textsubscript{area} and
}\emph{\DIFadd{R}}\DIFadd{\textsubscript{area}) versus LMA variation between sun and shade
due to metabolic leaf mass (with strong effects on
}\emph{\DIFadd{A}}\DIFadd{\textsubscript{area} and }\emph{\DIFadd{R}}\DIFadd{\textsubscript{area}). More
...
leaf lifespan (\mbox{%DIFAUXCMD
\citeproc{ref-Simonin2012}{Simonin et al. 2012}}\hspace{0pt}%DIFAUXCMD
) versus
metabolic LMA variation associated with photosynthetic responses to
changing nutrient conditions (\mbox{%DIFAUXCMD
\citeproc{ref-Walker2014a}{Walker et al.
2014}}\hspace{0pt}%DIFAUXCMD
). Because structural and metabolic LMA components appear largely
...
was supported by \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the Xingdian Talent Support Program of Yunnan Province,
the }\DIFaddend Xishuangbanna State Rainforest Talent Support Program \DIFdelbegin \DIFdel{, a }\DIFdelend \DIFaddbegin \DIFadd{(E4BN041B01),
the }\DIFaddend CAS President's International Fellowship Initiative (2020FYB0003),
\DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend ZiHui (Wisdom) Yunnan Program (202203AM140026), and \DIFdelbegin \DIFdel{a
...
\DIFaddbegin \bibitem[\citeproctext]{ref-Diaz2016}
\DIFadd{Díaz et al. (2016) The global spectrum of plant form and function.
Nature529:167--171. }\url{https://doi.org/10.1038/nature16489}\DIFadd{.
}

\DIFaddend \bibitem[\citeproctext]{ref-Dong2017}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-Simonin2012}
\DIFadd{Simonin et al. (2012) Hydraulic conductance of leaves correlates with
leaf lifespan: }{\DIFadd{Implications}} \DIFadd{for lifetime carbon gain. New
Phytologist193:939--947.
}\url{https://doi.org/10.1111/j.1469-8137.2011.04014.x}\DIFadd{.
}

\DIFaddend \bibitem[\citeproctext]{ref-Tcherkez2017}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-Walker2014a}
\DIFadd{Walker et al. (2014) The relationship of leaf photosynthetic traits --
}{\DIFadd{Vcmax}} \DIFadd{and }{\DIFadd{Jmax}} \DIFadd{-- to leaf nitrogen, leaf phosphorus, and specific
leaf area: A meta-analysis and modeling study. Ecology and
Evolution4:3218--3235. }\url{https://doi.org/10.1002/ece3.1173}\DIFadd{.
}

\DIFaddend \bibitem[\citeproctext]{ref-Westoby2000}
...
\textbf{Fig. 1:} \DIFdelbegin \DIFdel{Simulated }\DIFdelend \DIFaddbegin \DIFadd{Hypothetical }\DIFaddend two-dimensional \DIFdelbegin \DIFdel{functional }\DIFdelend \DIFaddbegin \DIFadd{trait }\DIFaddend space that can
\DIFdelbegin \DIFdel{result in either a }\DIFdelend \DIFaddbegin \DIFadd{appear as either }\DIFaddend two- or one-dimensional\DIFdelbegin \DIFdel{trait space}\DIFdelend , depending on how metabolic
...
traits). \DIFdelbegin \DIFdel{Panels b and c represent normalization by leaf area
and leaf mass, respectively}\DIFdelend \DIFaddbegin \DIFadd{LMAm and LMAs were sampled from independent normal
distributions}\DIFaddend . (b) Two-dimensional relationship between photosynthetic
...
\emph{A}\textsubscript{max} per-unit leaf mass and total LMA. \DIFdelbegin \DIFdel{Methods used to simulate the data are explained in Supplementary Section
5.
}\DIFdelend \DIFaddbegin \DIFadd{Whole-leaf
}\emph{\DIFadd{A}}\DIFadd{\textsubscript{max} values are identical in (b) and (c).
Distributions of LMAm and LMAs (a) and relationships between
}\emph{\DIFadd{A}}\DIFadd{\textsubscript{max} and LMA (b-c) were parameterized using the
