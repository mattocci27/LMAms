...
%DIF DEL LMAms_main_old.tex   Tue Aug 20 07:42:43 2024
%DIF ADD LMAms_main.tex       Tue Aug 20 07:41:31 2024
...
components (Appendices S1-S2). \DIFdelbegin \DIFdel{By using power-law relationships, our
...
observed trait data (Appendix S3). \DIFaddbegin \DIFadd{We then used the model predictions to
...
\emph{A}\textsubscript{max} and LMA (Appendix \DIFdelbegin \DIFdel{S4}\DIFdelend \DIFaddbegin \DIFadd{S5}\DIFaddend ), we \DIFdelbegin \DIFdel{simulated datasets
varying in }\DIFdelend \DIFaddbegin \DIFadd{generated
simulated datasets with different }\emph{\DIFadd{A}}\DIFadd{\textsubscript{area}-LMAm
relationships and different }\DIFaddend LMAm and LMAs \DIFdelbegin \DIFdel{(Appendix S8). This approach allows us to
directly examine the cause of }\DIFdelend \DIFaddbegin \DIFadd{variances (Appendix S6). For
each simulated dataset, we quantified the }\DIFaddend mass-dependence of
...
\DIFdelbegin \DIFdel{S4}\DIFdelend \DIFaddbegin \DIFadd{S5}\DIFaddend ) following Osnas et al. (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
simulated and actual datasets (Appendix S6).
These analyses are key in
...
of these assumptions is described in }\DIFaddend Table 1 and Appendix S2\DIFdelbegin \DIFdel{for more detail.
}%DIFDELCMD < 
...
the sun:shade LL ratio}\DIFdelend .
...
(\mbox{%DIFAUXCMD
\citeproc{ref-Osnas2018}{2018}}\hspace{0pt}%DIFAUXCMD
)). Generally}\DIFdelend \DIFaddbegin \DIFadd{see Appendix
S4). As expected}\DIFaddend , mass-dependence of \emph{A}\textsubscript{max} \DIFdelbegin \DIFdel{decreased }\DIFdelend \DIFaddbegin \DIFadd{was
generally weak }\DIFaddend (area-dependence \DIFdelbegin \DIFdel{increased) with
increasing LMAs variance }\DIFdelend \DIFaddbegin \DIFadd{was strong) when LMA variance was
dominated by LMAs }\DIFaddend (Fig.~\ref{fig-mass_prop}). \DIFdelbegin \DIFdel{Other factors,
including the sensitivity of }\DIFdelend \DIFaddbegin \DIFadd{In this case,
}\DIFaddend \emph{A}\textsubscript{max} per-unit leaf area
(\emph{A}\textsubscript{area}) \DIFaddbegin \DIFadd{and LMA are not strongly correlated
...
}\emph{\DIFadd{A}}\DIFadd{\textsubscript{max} thus depends primarily on leaf area rather
than mass).
}
...
%DIFDELCMD < (\citeproc{ref-Osnas2018}{Osnas et al., 2018}). See Appendices S9-S6 for
...
Appendices S4-S6 for details.}\DIFaddendFL }
