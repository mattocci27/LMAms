...
%DIF DEL LMAms_main_old.tex   Thu Jan 23 16:38:01 2025
%DIF ADD LMAms_main.tex       Thu Jan 23 16:36:26 2025
...
(Figs. S1a-b). The higher total LMA in evergreen compared to deciduous
leaves was primarily due to differences in LMAs (Figs. \ref{fig-box_de},
S2a, and S3). In the Panama dataset, light (sun vs.~shade) explained
more variation in LMAm than LMAs, whereas site (wet vs.~dry) explained
near zero variation in LMAm but substantial variaion in LMAs (Fig. S1c).
...
\ref{fig-box_pa} and S2b). In contrast, Panama sun leaves had higher
...
and S2b). LMAs comprised a larger fraction of total LMA for shade leaves
than for sun leaves (Fig. S4).
...
(LMAs)---each conferring different functions. By explicitly decomposing
}\DIFaddend LMA into LMAm and LMAs\DIFdelbegin \DIFdel{(one latent variable
...
model-inferred estimates of LMAm and LMAs}\DIFdelend \DIFaddbegin \DIFadd{, we reveal why mass‐based correlations that seem
to support a single LES axis can obscure key mechanisms and lead to
divergent area‐ vs.~mass‐dependence of photosynthetic traits across
species and environments. Far from rejecting the utility of the LES, our
...
diversity. This refined understanding has implications for both
ecological research and modeling frameworks---particularly dynamic
global vegetation models---that rely on trait‐based parameters to
predict ecosystem processes}\DIFaddend .　
...
vs.~\textasciitilde40\% for deciduous species }\DIFdelend \DIFaddbegin \DIFadd{By separating total LMA into these
...
mass compared with deciduous species and sun leaves---patterns broadly
consistent with published measurements of cell wall (fiber) content
(\mbox{%DIFAUXCMD
...
) }\DIFaddend (Fig. S3)\DIFdelbegin \DIFdel{, and
\textasciitilde70\% for shade leaves vs. ~\textasciitilde40\% for sun
leaves }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend (Fig. S4)
\DIFdelbegin \DIFdel{. These estimates align with measurements of cell wall
...
positions, with higher values for shade leaves; Kitajima et al.~2016)}\DIFdelend \DIFaddbegin \DIFadd{This correspondence supports our inference that structural investment,
rather than maintenance metabolism, drives leaf toughness and
persistence under conditions favoring long-lived leaves (e.g., shaded
understories or evergreen habits). In contrast, leaves exposed to high
light have a larger fraction of metabolically active tissue, which
drives their higher Rarea but reduces the fraction of mass devoted to
structure. Hence, decomposing LMA into metabolic and structural
components helps clarify why Rarea correlates tightly with LMAm yet
remains unassociated with the structural fraction of leaf mass}\DIFaddend .
...
;
\mbox{%DIFAUXCMD
\citeproc{ref-Niinemets2006}{\textbf{Niinemets2006?}}}\hspace{0pt}%DIFAUXCMD
). This likely
contributes to their low LMAs (Fig.~\ref{fig-pa_point}i)}\DIFaddend , \DIFdelbegin \emph{\DIFdel{R}}%DIFAUXCMD
...
different }\DIFdelend \DIFaddbegin \DIFadd{highlighting
that growth form adds another dimension to LMA variation, beyond the
metabolic--structural axis captured by our model. Herbaceous species may
invest less in mechanical toughness---potentially favoring rapid leaf
turnover---whereas woody plants often rely on thick‐walled fibers for
longevity under stress. Recognizing that LMA spans a range of densities
and chemical compositions thus reinforces the importance of decomposing
LMA into both metabolic and }\DIFaddend structural components to \DIFdelbegin \DIFdel{specific functions (e.g., mesophyll
...
canopy positions as well as ecophysiological tradeoffs across edaphic-climate gradients}\DIFdelend \DIFaddbegin \DIFadd{more accurately
model leaf function across diverse plant forms}\DIFaddend .
...
S2), which should lead to strong mass-dependence of
...
and S2), which may reflect the additional LMAs needed to protect and
...
\DIFdelbegin \DIFdel{For over two decades, low-dimensional trait tradeoffs have been explored
as a means to represent }\DIFdelend \DIFaddbegin \DIFadd{Trait‐based approaches have long been used to incorporate }\DIFaddend plant
functional diversity \DIFdelbegin \DIFdel{in the vegetation
components of biogeochemical models (\mbox{%DIFAUXCMD
...
goal of adding ecological andphysiological realism to coupled }\DIFdelend \DIFaddbegin \DIFadd{into biogeochemical models and, by extension, }\DIFaddend Earth
system models (\DIFdelbegin \DIFdel{ESMs, \mbox{%DIFAUXCMD
...
\citeproc{ref-Verheijen2013}{Verheijen et al. 2013}}\hspace{0pt}%DIFAUXCMD
;
}\DIFaddend \citeproc{ref-Wullschleger2014}{Wullschleger et al. 2014}). Recently,
...
\citeproc{ref-DantasdePaula2021}{Dantas de Paula et al. 2021})\DIFaddbegin \DIFadd{, }\DIFaddend along
with demographic approaches \DIFdelbegin \DIFdel{capable of representing }\DIFdelend \DIFaddbegin \DIFadd{that capture }\DIFaddend the dynamics of diverse
communities in ESMs (\citeproc{ref-Fisher2018}{Fisher et al. 2018}).
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Although many models employ the }\DIFaddend leaf economics spectrum (LES) \DIFdelbegin \DIFdel{has been interpreted as a single dominant axis of global leaf functional }\DIFdelend \DIFaddbegin \DIFadd{to
...
major axes of leaf function (r s\_fig(``LMAm\_LMAs''))}\DIFaddend . Consistent with
our conclusion that leaf function is \DIFdelbegin \DIFdel{multi-dimensional,
analyses of leaf nitrogen indicate
significant independent contributions of }\DIFdelend \DIFaddbegin \DIFadd{inherently multidimensional,
previous work also shows that nitrogen is partitioned into }\DIFaddend metabolic
(e.g., rubisco) \DIFdelbegin \DIFdel{and
structural leaf nitrogen components }\DIFdelend \DIFaddbegin \DIFadd{versus structural fractions
}\DIFaddend (\citeproc{ref-Dong2017}{Dong et al. 2017},
...
can then improve trait‐based predictions of plant performance, community
...
and leaf mass, respectively. (b) Two-dimensional relationship between
photosynthetic capacity (\emph{A}\textsubscript{max}) per-unit leaf area
and total LMA (equal to LMAm + LMAs). (c) One-dimensional relationship
between \emph{A}\textsubscript{max} per-unit leaf mass and total LMA.
