...
%DIF DEL LMAms_main_old.tex   Wed Aug 21 10:01:50 2024
%DIF ADD LMAms_main.tex       Wed Aug 21 10:01:08 2024
...
(LMAs: parameter \(\alpha_s\))}\DIFdelend \DIFaddbegin \DIFadd{LMAm then LMAs}\DIFaddend ,
and (ii) LL depends more strongly on LMAs \DIFdelbegin \DIFdel{(\(\beta_s\) in Eq. 4 than LMAm(\(\beta_m\)). The first assumption
...
constraint \(\beta_s\) \textgreater{} \(\beta_m\). See }\DIFdelend \DIFaddbegin \DIFadd{than LMAm. The implementation
of these assumptions is described in }\DIFaddend Table 1 and Supplementary Section
\DIFdelbegin \DIFdel{2 for more detail.
}%DIFDELCMD < 
...
(which implies }\DIFdelend \DIFaddbegin \DIFadd{(impliying }\DIFaddend an exponential increase in
