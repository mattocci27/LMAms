...
To clarify, the models were fit to *A*~area~ or *R*~area~, LMA, and LL (not *N*~area~, *P*~area~, or CL~area~).
This is explained on lines ~XXX, and we have now added an additional clarification of this point on lines ~XXX.
...
Good point.
We have changed ‘improved statistical fits’ to ‘improved predictions and insights’ (line XXX).
Similarly, in the next sentence, we changed ‘explain variation in other traits’ to ‘predict and understand variation in other traits’ (line XXX).
...
We now discuss how this supports a physiological basis for low LMAs in the revised text (Line XXX).
...
These points are now addressed in Discussion, where we added new text to highlight the broader significance (lines XXX and XXX) and specific implications for ecosystem models (lines XXX; e.g., biased representation of leaf respiration in single-axis approaches).
...
We have clarified in the figure caption that Fig. 1 illustrates simulated data drawn from normal distributions, displayed as points to mimic empirical sampling (now explained on lines XXX).
...
We think it is more helpful to show the simulation results with a sample of points, analogous to an empirical trait dataset (we have now clarified this in the figure caption on lines XXX).
