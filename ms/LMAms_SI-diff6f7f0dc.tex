...
%DIF DEL /var/folders/ph/r5fc9nzd1j38q4j50kgwd4cw0000gn/T/_ioryPXcx7/latexdiff-vc-6f7f0dc/ms/LMAms_SI.tex   Thu Mar 30 17:59:20 2023
%DIF ADD ms/LMAms_SI.tex                                                                                    Fri Mar 31 17:56:30 2023
...
\DIFdelend \DIFaddbegin \renewenvironment{Shaded}{\begin{tcolorbox}[sharp corners, borderline west={3pt}{0pt}{shadecolor}, interior hidden, enhanced, boxrule=0pt, frame hidden, breakable]}{\end{tcolorbox}}\DIFaddend \fi
...
\emph{A}\textsubscript{max}) \DIFaddbegin \DIFadd{as well as to plot Fig. 1}\DIFaddend , we generated
\DIFdelbegin \DIFdel{lognormally }\DIFdelend \DIFaddbegin \DIFadd{log-normally }\DIFaddend distributed LMAm and LMAs values by sampling ln(LMAm) and
ln(LMAs) from normal distributions.
...
\[
\mathrm{ln}(\mathrm{LMAm}) \sim N(\mathrm{ln}(\mu_m), \sigma_m) \DIFaddbegin \DIFadd{\qquad(\mathrm{S}3.1)
}\DIFaddend \]

\[
\mathrm{ln}(\mathrm{LMAs}) \sim N(\mathrm{ln}(\mu_s), \sigma_s) \DIFaddbegin \DIFadd{\qquad(\mathrm{S}3.2)
}\DIFaddend \]
...
= 0.83, \DIFaddbegin \DIFadd{\(\alpha_0\) = 1.77, }\DIFaddend \(\alpha_m\) = 0.28, and \(\alpha_s\) =
-0.13, for GLOPNET; \(\mu_m\) = 47.3, \(\mu_s\) = 30.1, \(\sigma_m\) =
1.58, \DIFaddbegin \DIFadd{\(\alpha_0\) = 0.34, }\DIFaddend \(\alpha_m\) = 0.56, and \(\alpha_s\) = 0,
for Panama sun leaves; and \(\mu_m\) = 7.6, \(\mu_s\) = 26.7,
\(\sigma_m\) = 1.95, \DIFaddbegin \DIFadd{\(\alpha_0\) = 0.34, }\DIFaddend \(\alpha_m\) = 0.56,
\(\alpha_s\) = 0, and \(\rho\) = -0.47 \DIFdelbegin \DIFdel{. }\DIFdelend for Panama shade leaves.

For each simulated dataset, we quantified the mass-dependence (\emph{b})
using the ordinary least squares regression from Eq. 5. We repeated
these steps 1000 times for each set of parameter values.

\DIFaddbegin \DIFadd{We also used Eqs. S3.1-S3.2 to create the simulated GLOPNET LMA datasets
in Fig. 1. }\emph{\DIFadd{A}}\DIFadd{\textsubscript{max} values were generated using Eq. 2
and Eq. S1. Based on our analysis of the GLOPNET database, the standard
deviations of ln(LMAs) and ln(}\emph{\DIFadd{A}}\DIFadd{\textsubscript{area}) were set to
1 and 0.31, respectively. The rest of the parameters were listed above.
}
