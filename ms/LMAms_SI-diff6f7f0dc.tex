...
%DIF DEL /var/folders/ph/r5fc9nzd1j38q4j50kgwd4cw0000gn/T/I_0KvGj4aT/latexdiff-vc-6f7f0dc/ms/LMAms_SI.tex   Thu Mar 30 17:59:20 2023
%DIF ADD ms/LMAms_SI.tex                                                                                    Sun Apr  2 10:48:50 2023
...
\begin{document}
\maketitle
\ifdefined\Shaded\DIFdelbegin %DIFDELCMD < \renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, enhanced, boxrule=0pt, breakable, interior hidden, frame hidden, sharp corners]}{\end{tcolorbox}}%%%
\DIFdelend \DIFaddbegin \renewenvironment{Shaded}{\begin{tcolorbox}[sharp corners, borderline west={3pt}{0pt}{shadecolor}, interior hidden, boxrule=0pt, frame hidden, enhanced, breakable]}{\end{tcolorbox}}\DIFaddend \fi
...
\DIFdelbegin \DIFdel{\({\mathbf \Sigma} = \mathrm {diag}(\mathbf {\sigma}){\mathbf \Omega}\mathrm {diag}({\mathbf \sigma}) = \mathrm {diag}({\mathbf \sigma}){\mathbf L}{\mathbf L}\prime \mathrm {diag}({\mathbf \sigma})\)
}\DIFdelend \DIFaddbegin \DIFadd{\({\boldsymbol \Sigma} = \mathrm {diag}(\boldsymbol {\sigma}){\boldsymbol \Omega}\mathrm {diag}({\boldsymbol \sigma}) = \mathrm {diag}({\boldsymbol \sigma}){\boldsymbol L}{\boldsymbol L}\prime \mathrm {diag}({\boldsymbol \sigma})\)
}\DIFaddend using a Cholesky decomposition, where \DIFdelbegin \DIFdel{\(\mathbf {\sigma}\) }\DIFdelend \DIFaddbegin \DIFadd{\(\boldsymbol {\sigma}\) }\DIFaddend is a
vector of \(\sigma_{1}\), \(\sigma_{2}\), and \(\sigma_{3}\);
\DIFdelbegin \DIFdel{\({\mathbf \Omega}\) }\DIFdelend \DIFaddbegin \DIFadd{\({\boldsymbol \Omega}\) }\DIFaddend is a correlation matrix of \(\rho_{12}\),
...
\DIFdelbegin \DIFdel{\({\mathbf \sigma}\) }\DIFdelend \DIFaddbegin \DIFadd{\({\boldsymbol \sigma}\) }\DIFaddend and \textbf{L} to avoid a strong dependence
between \DIFdelbegin \DIFdel{\({\mathbf \sigma}\) and \({\mathbf \Omega}\)
}\DIFdelend \DIFaddbegin \DIFadd{\({\boldsymbol \sigma}\) and \({\boldsymbol \Omega}\)
}\DIFaddend (\protect\hyperlink{ref-Alvarez2014}{Alvarez et al., 2014};
...
A prior for \DIFdelbegin \DIFdel{\(\mathbf{\sigma}\) }\DIFdelend \DIFaddbegin \DIFadd{\(\boldsymbol{\sigma}\) }\DIFaddend was specified as a Half-Cauchy
...
lifespan (\(\theta\)) were not significantly different from \DIFdelbegin \DIFdel{zeroin }\DIFdelend \DIFaddbegin \DIFadd{zero in }\DIFaddend the
...
photosynthetic capacity and LMA \DIFdelbegin \DIFdel{(i.e., mass- vs.~area-dependence of
}\emph{\DIFdel{A}}%DIFAUXCMD
\DIFdel{\textsubscript{max}), we generated lognormally }\DIFdelend \DIFaddbegin \DIFadd{, we generated log-normally }\DIFaddend distributed
LMAm and LMAs values by sampling ln(LMAm) and ln(LMAs) from normal
distributions.
\DIFdelbegin \DIFdel{For GLPONET }\DIFdelend \DIFaddbegin 

\DIFadd{For GLOPNET }\DIFaddend and Panama sun leaves, results from the LMAm-LMAs models
...
\[
\mathrm{ln}(\mathrm{LMAm}) \sim \DIFdelbegin \DIFdel{N}\DIFdelend \DIFaddbegin \DIFadd{\mathrm{N}}\DIFaddend (\mathrm{ln}(\mu_m), \sigma_m) \DIFaddbegin \DIFadd{\qquad(\mathrm{S}3.1)
}\DIFaddend \]

\[
\mathrm{ln}(\mathrm{LMAs}) \sim \DIFdelbegin \DIFdel{N}\DIFdelend \DIFaddbegin \DIFadd{\mathrm{N}}\DIFaddend (\mathrm{ln}(\mu_s), \sigma_s) \DIFaddbegin \DIFadd{\qquad(\mathrm{S}3.2)
}\DIFaddend \]

where \(\mathrm{ln}(\mu_m)\) and \(\mathrm{ln}(\mu_s)\) are the means of
LMAm and LMAs, respectively, on the log-scale and \(\sigma_m\) and
\(\sigma_s\) are the corresponding standard deviations. \DIFaddbegin \DIFadd{The parameters
...
\DIFdelbegin \DIFdel{\mathbf{\Sigma}
}\DIFdelend \DIFaddbegin \boldsymbol{\Sigma}
\DIFaddend \right] \DIFaddbegin \DIFadd{\qquad(\mathrm{S}3.3)
}\DIFaddend \]

\[
\DIFdelbegin \DIFdel{\mathbf{\Sigma} }\DIFdelend \DIFaddbegin \boldsymbol{\Sigma} \DIFaddend = \
\begin{bmatrix}
\sigma_m^2 & \rho \sigma_m \sigma_s \\
\rho \sigma_m \sigma_s & \sigma_s^2 \\
\end{bmatrix} \DIFaddbegin \DIFadd{\qquad(\mathrm{S}3.4)
}\DIFaddend \]
...
and ln(LMAs). \DIFaddbegin \DIFadd{The parameters in \(\boldsymbol{\Sigma}\) were all
...
\(\sigma_m\), and \(\rho\); and \DIFaddbegin \DIFadd{we }\DIFaddend varied \(\sigma_s\) from ln(1.01) to
...
Parameter values were \DIFdelbegin \DIFdel{, }\DIFdelend \(\mu_m\) = 56.5, \(\mu_s\) = 61.2, \(\sigma_m\) =
0.83, \DIFaddbegin \DIFadd{\(\alpha_0\) = 1.77, }\DIFaddend \(\alpha_m\) = 0.28, and \(\alpha_s\) = -0.13
\DIFdelbegin \DIFdel{, }\DIFdelend for GLOPNET; \(\mu_m\) = 47.3, \(\mu_s\) = 30.1, \(\sigma_m\) = 1.58,
\DIFaddbegin \DIFadd{\(\alpha_0\) = 0.34, }\DIFaddend \(\alpha_m\) = 0.56, and \(\alpha_s\) = 0 \DIFdelbegin \DIFdel{, }\DIFdelend for
Panama sun leaves; and \(\mu_m\) = 7.6, \(\mu_s\) = 26.7, \(\sigma_m\) =
1.95, \DIFaddbegin \DIFadd{\(\alpha_0\) = 0.34, }\DIFaddend \(\alpha_m\) = 0.56, \(\alpha_s\) = 0, and
\(\rho\) = -0.47 \DIFdelbegin \DIFdel{. }\DIFdelend for Panama shade leaves.

For each simulated dataset, we quantified the mass-dependence (\emph{b})
using the ordinary least squares regression \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{(log-log) form of }\DIFaddend Eq. 5. We
repeated these steps 1000 times for each set of parameter values.

\DIFaddbegin \DIFadd{We also used Eqs. S3.1-S3.2 to create the simulated LMA datasets in Fig.
1, which is based on our analysis of GLOPNET data.
}\emph{\DIFadd{A}}\DIFadd{\textsubscript{area} values were generated using Eq. 2 and Eq.
S1. Based on our GLOPNET results, the standard deviations of ln(LMAs)
and ln(}\emph{\DIFadd{A}}\DIFadd{\textsubscript{area}) were set to 1 and 0.31,
...
1) \DIFaddbegin \DIFadd{and Appendix S3}\DIFaddend : \(\alpha_0\) = 1.77, \(\mu_m\) = 56.5, \(\mu_s\) =
...
model (Table 1) \DIFaddbegin \DIFadd{and Appendix S3}\DIFaddend : \(\alpha_0\) = 0.34, \(\alpha_m\) =
