...
\section{Appendix S4: Variance
partitioning}\label{appendix-s4-variance-partitioning}

To estimate the contributions of LMAm and LMAs to total LMA variance
(where LMA = LMAm + LMAs), we used the following identity:

\[
\mathrm{Var}(Y = X1 + X2) = \mathrm{Cov}(Y, X1+X2) = \mathrm{Cov}(Y,X1) + \mathrm{Cov}(Y,X2) \qquad(\mathrm{S}4)
\]
...
\section{Appendix S5: Understanding relationships between photosynthetic
capacity and
...
\[
A_{\mathrm{area} \, i} = c (LMA_i)^{b}\epsilon_i \qquad(\mathrm{S}5)
\]
...
LMAm and LMAs for Panama shade leaves (see Appendix S6). Therefore, we
...
\section{Appendix S6: Simulated datasets to study relationships between
photosynthetic capacity and
LMA}\label{appendix-s6-simulated-datasets-to-study-relationships-between-photosynthetic-capacity-and-lma}
...
\[
\mathrm{ln}(\mathrm{LMAm}) \sim \mathrm{N}(\mathrm{ln}(\mu_m), \sigma_m) \qquad(\mathrm{S}6.1)
\]

\[
\mathrm{ln}(\mathrm{LMAs}) \sim \mathrm{N}(\mathrm{ln}(\mu_s), \sigma_s) \qquad(\mathrm{S}6.2)
\]
...
\end{bmatrix} \qquad(\mathrm{S}6.4)
\]
...
(Table 1) and Appendix S6: \(\alpha_0\) = 1.77, \(\mu_m\) = 56.4,
...
the best Panama model (Table 1) and Appendix S6: \(\alpha_0\) = 0.34,
