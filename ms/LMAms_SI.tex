...
\emph{A}\textsubscript{max}) as well as to plot Fig. 1, we generated
log-normally distributed LMAm and LMAs values by sampling ln(LMAm) and
...
= 0.83, \(\alpha_0\) = 1.77, \(\alpha_m\) = 0.28, and \(\alpha_s\) =
-0.13, for GLOPNET; \(\mu_m\) = 47.3, \(\mu_s\) = 30.1, \(\sigma_m\) =
1.58, \(\alpha_0\) = 0.34, \(\alpha_m\) = 0.56, and \(\alpha_s\) = 0,
for Panama sun leaves; and \(\mu_m\) = 7.6, \(\mu_s\) = 26.7,
\(\sigma_m\) = 1.95, \(\alpha_0\) = 0.34, \(\alpha_m\) = 0.56,
\(\alpha_s\) = 0, and \(\rho\) = -0.47 for Panama shade leaves.
...
We also used Eqs. S3.1-S3.2 to create the simulated GLOPNET LMA datasets
in Fig. 1. \emph{A}\textsubscript{max} values were generated using Eq. 2
and Eq. S1. Based on our analysis of the GLOPNET database, the standard
deviations of ln(LMAs) and ln(\emph{A}\textsubscript{area}) were set to
