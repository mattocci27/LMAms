...
{\centering \includegraphics{../figs/coef_rand.png}

}

\caption{\label{fig-rand}Although divergent transitions suggesting that
the posterior distribution is not reliable, we checked the regression
coefficients for GLOPNET (\(\alpha_{0, p, s}\), \(\beta_{0, s}\), and
...
\hypertarget{figure-s2-2}{%
\section{Figure S2}\label{figure-s2-2}}
...
\caption{\label{fig-mass_prop_sim}Mass\_prop.}

\end{figure}

\newpage

\hypertarget{figure-s3-1}{%
\section{Figure S3}\label{figure-s3-1}}
...
\caption{\label{fig-mass_prop_comp}Mass\_prop 2. hoge hoge.}
...
respectively) in the GLOPNET dataset. LMAp yields stronger correlations
and more consistent relationships compared to LMA and LMAs; e.g.,
evergreen and deciduous leaves align along a single relationship in
panel b, but not in panels a or c.}
