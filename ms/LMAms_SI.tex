...
Parameter settings are as following, GLOPNET: \(\mu_m\) = 58.8,
\(\mu_s\) = 63.1, \(\sigma_m\) = 2.12, \(\alpha_m\) = 0.28, \(\alpha_s\)
= -0.13, sun leaves in Panama: \(\mu_m\) = 45.1, \(\mu_s\) = 31,
\(\sigma_m\) = 1.53, and \(\alpha_m\) = 0.56, \(\alpha_s\) = 0, shade
leaves in Panama: \(\mu_m\) = 8.6, \(\mu_s\) = 24, \(\sigma_m\) = 1.88,
\(\alpha_m\) = 0.56, \(\alpha_s\) = 0, \(\rho\) = -0.4.
...
\textbf{Figure S1} Pearson correlation coefficients for posterior
medians of LMAm vs LMAs in the (a) GLOPNET and (b) Panama datasets. The
...
\textbf{Figure S2}: Boxplots comparing leaf mass per area (LMA),
photosynthetic leaf mass per area (LMAm; posterior means), and
structural leaf mass per area (LMAs; posterior means) across (a)
...
include all leaves in the Panama dataset, whereas Figures 6- 7 in the
main text only include Panama species for which both sun and shade
leaves were available. The best model in the table 1 were used. Boxplot
symbols as in Figures 6- 7. Groups sharing the same letters are not
...
\includegraphics{/Users/mattocci/Dropbox/MS/LMAms/figs/box_frac.png}

\textbf{Figure S3}: Boxplots comparing posterior medians of the latent
variable \emph{f} (the fraction of total LMA comprised by LMAm) across
deciduous (Dev) and evergreen (Eve) leaves. (a) GLOPNET dataset. (b) All
leaves for Panama species. (c) Leaves for Panama species for which both
sun and shade leaves were available. Note that LMAm = \emph{f}
\(\times\) LMA, and LMAs = (1 -- \emph{f}) \(\times\) LMA. The best
model in the table 1 were used. Boxplot symbols as in Figure 6. Groups
...
\textbf{Figure S4}: Boxplots comparing posterior medians of the latent
variable \emph{f} (the fraction of total LMA comprised by LMAm) across
...
as in Figure 7. Groups sharing the same letters are not significantly
...
\textbf{Figure S5}: The relationships between mass dependency (\emph{b}
in Equation 5 in the main text) and relative variance in LMAs to LMA for
...
0.28,). Solid lines indicate simulated means and shaded regions indicate
95\% CI. The photosynthetic rate (\emph{A}\textsubscript{max}) is
primarily mass-dependent (\emph{b} \textgreater{} 0.5), primarily
...
et al. 2018}). Note that when \emph{b} \textgreater{} 1,
\emph{A}\textsubscript{max} per-unit leaf area
(\emph{A}\textsubscript{area}) dramatically increase with LMA, which is
not realistic. Parameter settings: \(\alpha_0\) = 1.77, \(\mu_m\) =
58.8, \(\mu_s\) = 63.1, \(\sigma_m\) = 2.12.
...
\textbf{Figure S6}: The relationships between mass dependency (\emph{b}
in Equation 5 in the main text) and relative variance in LMAs to LMA for
...
= 0.33, \(\alpha_m\) = 0.56, \(\alpha_s\) = 0, \(\mu_m\) = 8.6,
\(\mu_s\) = 24, \(\sigma_m\) = 1.88, and \(\rho\) = -0.4,
...
\textbf{Figure S7}: Measured traits related to photosynthesis and
...
\textbf{Figure S8}: Partial regression plots for nitrogen, phosphorus
and cellulose per-unit leaf area (\emph{N}\textsubscript{area},
