...
](`r here("figs/ps_point.png")`){#fig-LMAp_LMAs}
...
![
Boxplots comparing posterior medians of the latent variable f (the fraction of total LMA comprised by LMAp) across deciduous (D) and evergreen (E) leaves in GLOPNET and Panama, and across sites (wet and dry) and canopy strata (sun and shade) in Panama. Note that LMAp = f × LMA, and LMAs = (1 – f) × LMA.
...
At the dry Panama site, the increase in LMA from shade to sun (Fig. \ref{fig-boxplt_pa}) is due to roughly equal proportional increases in LMAp and LMAs (because f is similar between sun and shade), whereas at the wet Panama site, the increase in LMA from shade to sun (Fig. \ref{fig-boxplt_pa}) is due primarily to increased LMAp (because f is greater in sun than shade).
