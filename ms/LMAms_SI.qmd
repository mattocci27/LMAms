...
Symbols as in Main Text Figures `r fig_num("gl_point")`-`r fig_num("pa_point")`.
...
**`r s_fig("box_inter")`**:
Boxplots comparing leaf mass per area (LMA), photosynthetic leaf mass per area (LMAm; posterior means), and structural leaf mass per area (LMAs; posterior means) across
(a) deciduous (Dev) and evergreen (Eve) leaves and
(b) sites (wet and dry) and canopy strata (sun and shade) in Panama.
The results shown here include all leaves in the Panama dataset, whereas Figures `r fig_num("box_de")`-`r fig_num("box_pa")` in the main text only include Panama species for which both sun and shade leaves were available.
The best model in the table 1 were used.
Boxplot symbols as in Figures `r fig_num("box_de")`-`r fig_num("box_pa")`.
...
(a) GLOPNET dataset.
(b) All leaves for Panama species.
(c) Leaves for Panama species for which both sun and shade leaves were available.
Note that LMAm = *f* $\times$ LMA, and LMAs = (1 – *f*) $\times$ LMA.
The best model in the table 1 were used.
Boxplot symbols as in Figure `r fig_num("box_de")`.
Groups sharing the same letters are not significantly different (P > 0.05; t-tests).
...
Note that LMAm = *f* $\times$ LMA, and LMAs = (1 – *f*) $\times$ LMA.
The best model in the table 1 were used.
Boxplot symbols as in Figure `r fig_num("box_pa")`.
Groups sharing the same letters are not significantly different (P > 0.05; t-tests).
...
Note that when *b* > 1, *A*~max~ per-unit leaf area (*A*~area~) dramatically increase with LMA, which is not realistic.
Parameter settings:
...
**`r s_fig("gl_point_np2")`**:
Measured traits related to photosynthesis and metabolism (nitrogen and phosphorus per-unit leaf area; *N*~area~ and *P*~area~) are positively correlated with LMA and with estimates (posterior means) of the metabolic and structural LMA components (LMAm and LMAs, respectively) in the GLOPNET dataset.
