...
P_vec1 <- paste("p[", 1:nrow(GL), ",1]" ,sep = "")
P_vec2 <- paste("p[", 1:nrow(GL), ",2]" ,sep = "")
temp_LMAp <- GL_summary[P_vec1, "mean"] * GL$LMA
temp_LMAs <- GL_summary[P_vec2, "mean"] * GL$LMA

GL <- GL %>%
  mutate(LMAp = temp_LMAp) %>%
  mutate(LMAs = temp_LMAs)  %>%
  mutate(LMAn = LMA - LMAp - LMAs)

par(mfrow = c(3,3))
plot(Aarea ~ LMAp,log = "xy" GL)
plot(Aarea ~ LMAs, log = "xy", GL)
plot(Aarea ~ LMAn, log = "xy", GL)
