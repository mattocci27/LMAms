...
<!-- 
This chunk checks how different ap and as change the relationship between mass-dependency and var(LMAs)>
-->
...
GL_sim <- \(LMAs, 
            para = c("alpha_s=0", 
                     "original", 
                     "alpha_p: double",
                     "alpha_p: double, alpha_s = 0")){
  if (para == "alpha_s=0") {
    A <- exp(log(GL_para$`X50.`[1]) +
             GL_para$`X50.`[2] * log(LMAp)
    )
  } else if (para == "alpha_p: double") {
    A <- exp(log(GL_para$`X50.`[1]) +
             GL_para$`X50.`[2] * 2 * log(LMAp) +
             GL_para$`X50.`[3] * log(LMAs)
    )
  } else if (para == "alpha_p: double, alpha_s = 0") {
    A <- exp(log(GL_para$`X50.`[1]) +
             GL_para$`X50.`[2] * 2 * log(LMAp)
    )
  } else {
...
mass_sim_fun <- \(data, name, para = c("alpha_s=0", 
                                       "original", 
                                       "alpha_p: double",
                                       "alpha_p: double, alpha_s = 0",
                                       )) {
  mu_GL <- mu_fun(data$LMAp)
  mu_GL2 <- mu_fun(data$LMAs)
  sig_GL <- sig_fun(data$LMAp)
  LMAs_var <- NULL
  b_GL <- NULL
  ...
  for (i in 1:1000) {
    ...
    LMAs <- map(tmp, \(x) rlnorm(N, mu_GL2, x))
    LMAs_var <- cbind(LMAs_var, map_dbl(LMAs, var_fun))
    b_GL <- cbind(b_GL, map_dbl(LMAs, GL_sim, para))
  }
  ...
  upr_GL <- apply(b_GL, 1, \(x)(quantile(x, 0.975)))
  lwr_GL <- apply(b_GL, 1, \(x)(quantile(x, 0.025)))
  tibble(mean = mean_GL, upr = upr_GL, lwr = lwr_GL,
         LMAs_var_mean,
  para = paste(para),
  site = paste(name))
...
tmp1 <- mass_sim_fun(GL, name = "GLOPNET", para = "alpha_s=0")
tmp2 <- mass_sim_fun(GL, name = "GLOPNET", para = "alpha_p: double")
tmp3 <- mass_sim_fun(GL, name = "GLOPNET", para = "alpha_p: double, alpha_s = 0")
tmp4 <- mass_sim_fun(GL, name = "GLOPNET", para = "original=0")

GL_sim <- bind_rows(tmp1, tmp2, tmp3, tmp4)
...
#                  fill = para),
#              alpha = 0.4)  +
  geom_line(aes(y = mean, x = LMAs_var_mean, col = para))
...
vpart_p <- vpart_GL + vpart_PA &
  ylab("Explained variance (%)") &
  xlab("")

my_ggsave("../figs/vpart.png", vpart_p,
          height = 6, width = 10)
```

![vpart](../figs/vpart.png)
...
GLboxFrac <- ggplot(GLFracDat, aes(x = gr, y = LMAp_frac, fill = gr)) +
  geom_boxplot(outlier.shape = 21, outlier.size = 1) +
  scale_fill_manual(values = fills, guide = "none") 
  #geom_text(data= lab1, aes(label = lab),
  #          vjust = -1,
...
PAboxFrac <- ggplot(PAFracDat, aes(x = gr, y = LMAp_frac, fill = gr)) +
  geom_boxplot(outlier.shape = 21, outlier.size = 1) +
  scale_fill_manual(values = fills, guide = "none") 
...
#lab3 <- FracDat %>%
#  group_by(gr, data) %>%
#  summarize(LMAp_frac = max(LMAp_frac)) %>%
#  ungroup %>%
#  arrange(data) %>%
#  mutate(lab = p_letters$Frac %>% unlist)

FracBox <- ggplot(FracDat, aes(x = gr, y = LMAp_frac, fill = gr, col = gr)) +
  geom_boxplot(outlier.shape = 21, outlier.size = 1) +
...
FracBox2 <- ggplot(FracDat3, aes(x = gr, y = LMAp_frac, fill = gr, col = gr)) +
  geom_boxplot(outlier.shape = 21, outlier.size = 1) +
...
my_ggsave("../figs/LMAp_paired.png", p,
          width = 11.7,
          height = 11.7)

```

![LMA-paired](../figs/LMAp_paired.png)
...
GL_LMAps <- ggplot(GL, aes(x = LMAp, y = LMAs, 
                               fill = gr, 
                               col = gr)) +
  geom_point(shape = 21) +
...
GL_LMAps95 <- ggplot(GL, aes(x = LMAp, y = LMAs, 
                               fill = gr, 
                               col = gr)) +
...
r_vals$r_vals$PA_LMAps$p_val
r_vals$r_vals$GL_LMAps$p_val

PA_LMAps95 <- ggplot(PA_tmp,
                   aes(x = LMAp, y = LMAs, 
                               fill = gr, 
                               col = gr)) +
  geom_segment(aes(x = LMAp_lo, xend = LMAp_up, y = LMAs, yend = LMAs), col = "grey60", size = 0.25) +
...
LMAps_plt <- GL_LMAps + PA_LMAps +
  plot_annotation(tag_levels = "a")

my_ggsave("../figs/LMAps.png", LMAps_plt,
          width = 11.7,
          height = 7)

LMAps_plt95 <- GL_LMAps95 + PA_LMAps95 +
  plot_annotation(tag_levels = "a")


my_ggsave("../figs/LMAps95.png", LMAps_plt95,
          width = 11.7,
          height = 7)

```

![LMAps](../figs/LMAps.png)

![LMAps95](../figs/LMAps95.png)
