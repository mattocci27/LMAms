...
GL2 <- GL_tb |>
  ...
  dplyr::select(data, model = model2, N, looic, alpha_s, beta_p)
...
PA2 <- PA_tb |>
  ...
  mutate(model3 = ifelse(str_detect(model, "opt"), paste0(model2, " + Light"), model2)) |>
  dplyr::select(data, model = model3, N, looic, alpha_s, beta_p)

bind_rows(GL2, PA2) |>
  mutate(no1 = case_when(
      str_detect(model, "LMA \\+ Light") ~ 3,
      str_detect(model, "LMAs \\+ Light") ~ 4,
      str_detect(model, "LT \\+ Light") ~ 5,
      str_detect(model, "LMAs") ~ 2,
  ...
  mutate(no3 = paste0(no1, no2)) |>
  dplyr::select(data, model = no3, N, looic) |>
  write_csv("../data/model_selection.csv")
