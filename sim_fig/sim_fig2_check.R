...
for (i in 1:1000){
  ...
  obs_cor_A[i] <- cor(log(LMAs_temp), log(A))
...
for (i in 1:10){
  ...
  for (j in 1:1000){
    ...
    LMAp_temp <- temp * LMA
    LMAs_temp <- LMA - LMAp_temp
    cor_A[j] <- cor(log(LMAs_temp), log(A))
...
p <- ggplot(moge[1:10000,], aes(x = cor_A, fill = model))
p <- p + geom_density(aes(y=0.1*..density..), alpha = 0.2) + theme_bw() +
