...
```{r,eval=FALSE}
targets::tar_load(gl_sim_summary)
targets::tar_load(pa_sim_summary)

gl_sim_summary |> str()
gl_sim_summary_branch <- NULL
pa_sim_summary_branch <- NULL
for (i in 1:10) {
  targets::tar_load(gl_sim_summary, i)
  if (i < 10) {
    gl_sim_summary$branch <- str_c("branch_0", i)
  } else {
    gl_sim_summary$branch <- str_c("branch_", i)
  }
  gl_sim_summary_branch <- bind_rows(gl_sim_summary_branch, gl_sim_summary)

  targets::tar_load(pa_sim_summary, i)
  if (i < 10) {
    pa_sim_summary$branch <- str_c("branch_0", i)
  } else {
    pa_sim_summary$branch <- str_c("branch_", i)
  }
  pa_sim_summary_branch <- bind_rows(pa_sim_summary_branch, pa_sim_summary)
}

gl_sim_summary_branch |>
  filter(rhat > 1.1) |>
  count(branch) |>
  full_join(gl_sim_summary_branch |>
    filter(variable == "lp__")) |>
  dplyr::select(branch, num_rhat = n, num_divergent)


pa_sim_summary_branch |>
  filter(rhat > 1.1) |>
  count(branch) |>
  full_join(pa_sim_summary_branch |>
    filter(variable == "lp__")) |>
  dplyr::select(branch, num_rhat = n, num_divergent)
