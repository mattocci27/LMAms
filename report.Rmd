...
```{r}
fit_1_summary_GL_LMA |> filter(rhat > 1.1)
fit_2_summary_PA_LMA |> filter(rhat > 1.1)
fit_3_summary_PA_LMA_opt |> filter(rhat > 1.1)
fit_4_summary_GL_Ap_LLs |> filter(rhat > 1.1)
fit_5_summary_GL_Aps_LLps |> filter(rhat > 1.1)
fit_6_summary_GL_Ap_LLps |> filter(rhat > 1.1)
fit_7_summary_GL_Aps_LLs |> filter(rhat > 1.1)
fit_8_summary_PA_Ap_LLs |> filter(rhat > 1.1)
fit_9_summary_PA_Aps_LLps |> filter(rhat > 1.1)
fit_10_summary_PA_Ap_LLps |> filter(rhat > 1.1)
fit_11_summary_PA_Aps_LLs |> filter(rhat > 1.1)
fit_12_summary_PA_Ap_LLs_opt |> filter(rhat > 1.1)
fit_13_summary_PA_Aps_LLps_opt |> filter(rhat > 1.1)
fit_14_summary_PA_Ap_LLps_opt |> filter(rhat > 1.1)
fit_15_summary_PA_Aps_LLs_opt |> filter(rhat > 1.1)
fit_16_summary_PA_Ap_LDs |> filter(rhat > 1.1)
fit_17_summary_PA_Ap_LDps |> filter(rhat > 1.1)
fit_18_summary_PA_Ap_LDs_opt |> filter(rhat > 1.1)
fit_19_summary_PA_Ap_LDps_opt |> filter(rhat > 1.1)
fit_20_summary_PA_Ap_LLs_opt |> filter(rhat > 1.1)
```
...
div_check(fit_12_diagnostics_PA_Ap_LLs_opt)
...
targets::tar_read(pa_rand_check)
