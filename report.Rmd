...
targets::tar_load(fit_beta_summary_GL_beta)
targets::tar_load(fit_h_summary_GL_h)
```
...
fit_beta_summary_GL_beta |> filter(rhat > 1.05) |> DT::datatable()
fit_h_summary_GL_h |> filter(rhat > 1.05)
```

```{r}
fit_h_summary_GL_h |> DT::datatable()
```
...
targets::tar_load(fit_beta_diagnostics_GL_beta)
targets::tar_load(fit_h_diagnostics_GL_h)
```
...
div_check(fit_beta_diagnostics_GL_beta)
div_check(fit_h_diagnostics_GL_h)
```
