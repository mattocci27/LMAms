...
# obs <- rmvnorm(100, mean = c(log(3), log(5), log(1.5)), sigma = Sigma)
AR <- exp(obs[,1])
LL <- exp(obs[,2])
R <- exp(obs[,3])
A <- AR - R
...
# save.image("~/Dropbox/LES/null_sim.RData")
save.image("/scratch/lfs/mattocci27/LES/null_sim_20160630.RData")
