...
model_unif_pot <- "
  ...
  generated quantities {
    ...
    real rho2;
    real rho3;
    vector<lower=0>[n_sample] LMAp;
    vector<lower=0>[n_sample] LMAs;
    cov_matrix[3] Sigma;
    ...
    LMAp <- p .* LMA;
    LMAs <- LMA - LMAp;
    rho1 <- mean((log(LMAp) - mean(log(LMAp))) .* (to_vector(obs[, 1]) - mean(obs[ , 1]))) / (sd(log(LMAp)) * sd(obs[, 1]));
    rho2 <- mean((log(LMAs) - mean(log(LMAs))) .* (to_vector(obs[, 2]) - mean(obs[ , 2]))) / (sd(log(LMAs)) * sd(obs[, 2]));
    rho3 <- mean((log(LMAp) - mean(log(LMAp))) .* (to_vector(obs[, 3]) - mean(obs[ , 3]))) / (sd(log(LMAp)) * sd(obs[, 3]));
...
system.time(res1 <- stan(fit = fit1,
           ...
           thin = n.thin,
           chains = n.chains,
           control = list(stepsize = 0.1, adapt_delta = 0.8,
              max_treedepth = 10)))
...
save.image("~/Dropbox/LES/GL_sim_add.RData")
