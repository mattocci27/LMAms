...
#ggplot(GL, aes(x = log(LMA), y = p_val, col = DE)) +
#  geom_point() +
...
beta <- summary6 %>% head(21) %>% .$X50. %>% matrix(ncol=3)
...
for (i in 1:16){
  mu_temp <- rmvnorm(1,
    mean = c(beta[1,1] + beta[2,1] * log(LMAp[i]) - 0.5 * sigma1^2,
        beta[3,1] + beta[4,1] * log(LMAs[i]) - 0.5 * sigma2^2,
        beta[5,1] + beta[6,1] * log(LMAp[i]) + beta[7,1] * log(LMAs[i]) - 0.5 * sigma3^2),
    sigma = Sigma
    )
...
for (i in 17:150){
  mu_temp <- rmvnorm(1,
    mean = c(beta[1,2] + beta[2,2] * log(LMAp[i]) - 0.5 * sigma1^2,
        beta[3,2] + beta[4,2] * log(LMAs[i]) - 0.5 * sigma2^2,
        beta[5,2] + beta[6,2] * log(LMAp[i]) + beta[7,2] * log(LMAs[i]) - 0.5 * sigma3^2),
    sigma = Sigma
    )
  obs <- rbind(obs, mu_temp)
}
for (i in 151:200){
  mu_temp <- rmvnorm(1,
    mean = c(beta[1,3] + beta[2,3] * log(LMAp[i]) - 0.5 * sigma1^2,
        beta[3,3] + beta[4,3] * log(LMAs[i]) - 0.5 * sigma2^2,
        beta[5,3] + beta[6,3] * log(LMAp[i]) + beta[7,3] * log(LMAs[i]) - 0.5 * sigma3^2),
    sigma = Sigma
    )
  obs <- rbind(obs, mu_temp)
}
...
lm(log(Aarea) ~ log(LMA), GL_sim_dat) %>% summary
lm(log(Aarea) ~ log(LMAp), GL_sim_dat) %>% summary


cov(GL_sim_dat$LMA, GL_sim_dat$LMAp)
cov(GL_sim_dat$LMA, GL_sim_dat$LMAs)
