...
fig_full <- captioner::captioner(prefix = "Figure")
table_full <- captioner::captioner(prefix = "Table")
eq_full <- captioner::captioner(prefix = "Equation")
s_fig_full <- captioner::captioner(prefix = "Figure S", auto_space = FALSE)
...
# |> doen't work
fig_num <- function (name) {fig(name) %>% str_remove("Figure ")}
table_num <- function (name) {table(name) %>% str_remove("Table ")}
eq_num <- function (name) {eq(name) %>% str_remove("Equation ")}
s_fig_num <- function (name) {s_fig(name) %>% str_remove("Figure ")}
